...
Al igual que en la I<shell>, en Perl se usa C<< "<" >> para abrir el archivo en modo de solo lectura.  Si se abre correctamente, Perl le asigna un nuevo identificador de archivo y coloca una referencia a ese identificador en el argumento C<$identificador> que estaba indefinido.
...
B<Nota sobre la codificación>: Puede resultar pesado tener que especificar la codificación del texto cada vez que se abre un archivo.  Con el I<pragma> C<open> se puede configurar una codificación predeterminada para la función C<open> de forma que no sea necesario indicarla en cada ocasión:
...
Para preservar el contenido actual hay que abrir el archivo en modo anexar.  Al igual que en la I<shell>, en Perl se usa C<<< ">>" >>> para abrir un archivo en modo anexar.  Si el archivo no existe, C<<< ">>" >>> lo crea.
...
En este caso Perl también funciona igual que la I<shell>: C<< ">" >> sobrescribe un archivo existente.
