...
Un array de un array es justo un array @AoA normal sobre la que se puede aplicar dos subíndices, como C<$AoA[3][2]>.  Aquí hay una declaración del array:
    ...
    use 5.010;  # así podemos usar say()
...
La razón de que no hace lo que usted quiere que haga es porque la asignación de un array con nombre como si fuera un escalar se toma como si se tomara un escalar de un array en contexto escalar, lo que significa que sólo obtenemos el número de elementos que hay en @tmp.
...
porque solo va a conseguir un listado de referencias, y perl nunca desreferencia de forma automática.  En su lugar, tiene que hacer un bucle o dos.  Esto muestra toda la estructura, utilizando la construcción de un bucle al estilo del for() de la I<shell> para ciclar por el conjunto externo de subíndices.
