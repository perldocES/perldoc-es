...
Esta es la documentación completa sobre todos los aspectos de las referencias.
Para una guía de introducción más corta, a sólo las características esenciales, véase L<perlreftut>.

=head1 DESCRIPCIÓN

Antes de la distribución de Perl 5, era difícil representar estructuras complejas de datos, porque todas las referencias tenían que ser simbólicas -e incluso entonces era difícil referirse a una variable en lugar de a una entrada de la tabla de símbolos-.
...
Cualquier escalar puede contener una referencia dura.  Dado que los arrays y los hashes contienen escalares, ahora puede fácilmente generar arrays de arrays, arrays de hashes, arrays de hashes de funciones, y así sucesivamente.

Las referencias duras son inteligentes: hacen por usted un seguimiento de los recuentos de referencias, liberando automáticamente lo que se refiere en cuanto su recuento de referencias llega a cero.  (Los recuentos de referencia para valores en estructuras de datos auto referenciadas o cíclicas quizás no puedan llegar a cero sin un poco de ayuda. Vea L<perlobj/"Recolección de basura en dos fases"> para una explicación detallada).
Si eso le pasa a un objeto, el objeto se destruye.  Vea L<perlobj> para más información sobre los objetos.  (En cierto sentido, todo en Perl es un objeto, pero por lo general se reserva la palabra para las referencias a objetos que han sido oficialmente "bendecidos" en una clase I<package>).
...
Por el contrario, las referencias duras son como los vínculos duros de un sistema de archivos de Unix: se utilizan para acceder a un objeto subyacente, sin preocuparse por lo que su (otro) nombre sea.  Cuando la palabra "referencia" se utiliza sin adjetivo, como en el párrafo siguiente, por lo general se trata de una referencia dura.
...
Pero vea la explicación de la sintaxis C<*foo{COSA}> más abajo.  Sin embargo, aún puede utilizar I<globs> y I<globrefs> como si fueran identificadores de E/S.
...
Por otro lado, si desea el otro significado, puede hacer esto:
    ...
    sub showem {        { @_ } }   # ambiguo (actualmente válido,
                                   # pero puede cambiar)
    sub showem {       {; @_ } }   # bien
...
Una referencia a una subrutina anónima se puede crear mediante el uso de un C<sub> sin nombre:

    $coderef = sub { print "Boink!\n" };

Preste atención al punto y coma.  Salvo porque el código en el interior no se ejecuta inmediatamente, una C<sub {}> no es tanto una declaración como un operador, al igual que C<do{}> o C<eval{}>.  (Sin embargo, no importa cuántas veces ejecute esa línea (a menos que esté en un C<eval("...")>), $coderef seguirá siendo una referencia a la I<misma> subrutina anónima).

Las subrutinas anónimas actúan como clausuras con respecto a las variables my(), es decir, variables léxicas visibles dentro del ámbito actual.  La clausura es una noción del mundo Lisp, que dice que si se define una función anónima en un particular contexto léxico, se pretende que se ejecute en ese contexto, incluso cuando se llama fuera de ese contexto.

En términos humanos, es una manera divertida de pasar argumentos a una subrutina cuando la define, así como cuando se le llama.  Es útil para la creación de pequeños trozos de código para ejecutarlos más tarde, como las retrollamadas.  Puede incluso hacer cosas orientadas a objetos con ello, a pesar de que Perl ya proporciona un mecanismo diferente para hacer eso -vea L<perlobj>-.
...
Por cierto, esto sólo se aplica a las variables léxicas.  Las variables dinámicas siguen funcionando como siempre lo han hecho.  La clausura no es algo en que la mayoría de los programadores de Perl tengan que empezar a entrometerse.
...
Las referencias del tipo apropiado pueden crearse de la nada si los desreferencia en un contexto en el que asume que existen.  Como no hemos hablado sobre la desreferenciación, todavía no podemos mostrarle ningún ejemplo.

=item 7.
X<*foo{COSA}> X<*>

Una referencia se puede crear mediante el uso de una sintaxis especial, cariñosamente conocida como la sintaxis *foo{COSA}.  *foo{COSA} devuelve una referencia a la entrada COSA en *foo (que es la entrada en la tabla de símbolos que contiene todo lo que se conoce como I<foo>).
...
Todos ellos se explican por sí mismos, excepto para C<*foo{IO}>, que requiere una atención especial.  Devuelve el identificador de E/S, utilizado para los identificadores de archivo (L<perlfunc/open>), I<sockets> (L<perlfunc/socket> and L<perlfunc/socketpair>), e identificadores de directorio (L<perlfunc/opendir>).  Por razones de compatibilidad con versiones anteriores de Perl, C<*foo{IDENTIFICADOR_ARCHIVO}> es un sinónimo de C<*foo{IO}>, a pesar de que no se recomienda, para usar en su lugar un uso consistente de un único nombre: IO.  En versiones de perl entre v5.8 y v5.22, emitirá una advertencia de obsolescencia, pero esta obsolescencia se rescindió.

C<*foo{COSA}> devuelve undef si esta COSA particular todavía no se ha usado, excepto en el caso de los escalares.  C<*foo{SCALAR}> devuelve una referencia a un escalar anónimo si $foo todavía no se ha usado.  Esto podría cambiar en una futura distribución.

C<*foo{NAME}> y C<*foo{PACKAGE}> son la excepción, en que devuelven cadenas de caracteres, en lugar de referencias.  Devuelven el paquete y el nombre del propio typeglob, en lugar del que le fue asignado.  Así que, después de C<*foo=*Foo::bar>, C<*foo> devolverá "*Foo::bar" cuando se usa como cadena, pero C<*foo{PACKAGE}> y C<*foo{NAME}> seguirán generando "main" y "foo", respectivamente.

C<*foo{IO}> es una alternativa al mecanismo C<*IDENTIFICADOR> indicado en L<perldata/"Typeglobs e identificadores de archivo"> para pasar identificadores de archivo en o fuera de las subrutinas, o almacenarse en grandes estructuras de datos.
...
Eso era todo lo que tenía que saber para la creación de referencias.  Por ahora se estará muriendo por saber cómo utilizar las referencias para obtener sus datos perdidos hace tiempo.  Hay varios métodos básicos.
...
Las llamadas a subrutinas y las búsquedas de elementos individuales en un array surgen con tanta frecuencia que se hace engorroso utilizar el método 2.  Como una forma sintácticamente edulcorada, los ejemplos para el método 2 se pueden escribir así:
...
Este es uno de los casos que hemos mencionado antes en el que las referencias podrían aparecer a la existencia cuando está en un contexto valor-izquierda.  Antes de esta declaración, C<$array[$x]> podría estar indefinida.  Si es así, es automáticamente definida con una referencia a un hash, para que podamos buscar C<{"foo"}> dentro de ella.  Asimismo C<< $array[$x]->{"foo"} >> quedará automáticamente definido con una referencia a un array para que podamos buscar C<[0]> dentro de él.
Este proceso se llama I<autovivificación>.
...
Un typeglob se puede desreferenciar de la misma manera que puede hacerlo una referencia, porque la sintaxis de desreferenciar siempre indica el tipo de referencia deseada.
...
Aquí hay un truco para la interpolación de una llamada a subrutina en una cadena:
    ...
    print "Mi sub retornó esta vez @{[misub(1,2,3)]}.\n";

Esto funciona así: cuando se ve un C<@{...}> en la cadena entre comillas dobles, se evalúa como un bloque.  El bloque crea una referencia a un array anónimo que contiene el resultado de la llamada a C<mysub(1,2,3)>.  Así que todo el bloque devuelve una referencia a un array, que luego se desreferencia por C<@{...}> y se pega en la cadena de comillas dobles. Esta argucia también es útil para expresiones arbitrarias:
...
La referencia de C<$foo> a C<$bar> se ha debilitado. Cuando la variable C<$bar> sale del ámbito, será recolectada. La próxima vez que miremos el valor de la clave de C<< $foo->{bar} >> será C<undef>.
...
A menudo la gente espera que funcione de esta manera.  Y así lo hace.
    ...
    &$nombre();                 # Llama a &foo()
    $pack = "ESE";
    ${"${pack}::$nombre"} = 5;  # Establece $ESE::foo, sin usar eval
...
Las llaves alrededor de una referencia simbólica solo sirve para aislar un identificador o nombre de variable del resto de una expresión, tal como siempre lo ha hecho dentro de una cadena.  Por ejemplo,

    $push = "pop on ";
    print "${push}over";

siempre ha significado imprimir "pop on over", incluso aunque push sea una palabra reservada.  Esto se generalizó para funcionar de la misma forma sin las comillas dobles que lo engloban, de modo que
...
tendrán el mismo efecto.  Esta construcción I<no> se considera una referencia simbólica cuando se está utilizando C<strict refs>:

    use strict 'refs';
    ${ bareword };      # Muy bien, significa $bareword.
    ${ "bareword" };    # Error, referencia simbólica.

Del mismo modo, debido a que todo subíndice se ha construido usando palabras sueltas, se aplica la misma regla a cualquier palabra suelta que se utilice como subíndice de un hash.
...
El I<pragma> C<use warnings> o la opción B<-w> le avisarán si se interpreta una palabra reservada como una cadena.
...
=head2 Pseudo-hashes: Usar un array como un hash
X<pseudo-hash> X<pseudo hash> X<pseudohash>

Los pseudo-hashes se han eliminado de Perl.  El I<pragma> 'fields' sigue estando disponible.
...
Como se explicó anteriormente, una función anónima con acceso a las variables léxicas visibles cuando esa función se compiló, crea una clausura.  Conserva el acceso a dichas variables, incluso aunque no se ejecuten hasta más tarde, como en un controlador de señal o una devolución de llamada Tk.
...
Las funciones roja y verde serían similares.  Para crearlas, vamos a asignar una clausura a un typeglob del nombre de la función que estamos tratando de generar.
...
Ahora interior() sólo se puede llamar desde dentro de exterior(), debido a las asignaciones temporales de la subrutina anónima. Pero cuando lo hace, tiene acceso normal a la variable léxica $x del ámbito exterior() en el momento en que exterior() se invoca.
...
=head1 Sintaxis de desreferencia posfija

A partir de v5.20.0, está disponible una sintaxis posfija para usar referencias.  Se comporta tal como se describe en L</Usar referencias>, pero en vez de usar un sigilo prefijado, se usa un sigilo-y-asterisco posfijo.

Por ejemplo:

    $r = \@a;
    @b = $r->@*; # equivale a @$r o @{ $r }

    $r = [ 1, [ 2, 3 ], 4 ];
    $r->[1]->@*;  # equivale a @{ $r->[1] }

En Perl 5.20 y 5.22, esta sintaxis se debe activar con C<use feature 'postderef'>. A partir de 5.24, no se requieren más declaraciones de características (I<feature>) para que quede disponible.

La desreferencia posfija debería funcionar en todas las circunstancias donde funcione un bloque de desreferencia (infijo), y debería ser completamente equivalente.  Esta sintaxis permite escribir y leer desreferencias izquierda a derecha de forma completa.  Se definen las siguientes equivalencias:

  $sref->$*;  # lo mismo que  ${ $sref }
...
Preste atención a que C<< $cref->&* >> I<no> es equivalente a C<< $cref->() >>, y puede servir para propósitos diferentes.

Los elementos Glob se pueden extraer por medio de la característica de la desreferencia posfija:

  $gref->*{SCALAR}; # lo mismo que *{ $gref }{SCALAR}

La desreferencia de array y escalar posfija se I<puede> usar en la interpolación de cadenas (comillas dobles o con el operador C<qq>), pero sólo si se activa la característica C<postderef_qq>.

=head2 Partición posfija de referencias

También se pueden tomar porciones de valores de arrays y hashes con la notación de desrefencia posfija, con las siguientes equivalencias:

  $aref->@[ ... ];  # lo mismo que @$aref[ ... ]
  $href->@{ ... };  # lo mismo que @$href{ ... }

La partición de pares clave/valor, añadido en 5.20.0 y documentado en L<la sección de particiones hash claveE<sol>valor de perldata|perldata/"Porciones hash clave/valor">, también se comporta como se espera:

  $aref->%[ ... ];  # lo mismo que %$aref[ ... ]
  $href->%{ ... };  # lo mismo que %$href{ ... }

Igual que con la desreferencia posfija de array, la desreferencia posfija de porciones de valores se I<puede> usar en la interpolación de cadenas (comillas dobles o con el operador C<qq>), pero sólo si se activa la L<feature> (característica) C<postderef_qq>.

=head1 Asignar a referencias

A partir de v5.22.0, se puede asignar valores al operador de referencia.  Realiza una operación de alias, por lo que el nombre de la variable referenciada en la parte izquierda se convierte en un alias de la cosa referenciada en la parte derecha:

    \$a = \$b; # $a y $b apuntan ahora al mismo escalar
    \&foo = \&bar; # foo() ahora quiere decir bar()

La sintaxis se debe activar con C<use feature 'refaliasing'>.  Es experimental, y por defecto lanzará un aviso, a menos que se active C<no warnings 'experimental::refaliasing'>.

Se pueden asignar a estas formas, y hacen que la parte derecha se evalúe en contexto escalar:
    ...
    \my @array
    \my %hash
    \state $escalar # o @array, etc.
    \our $escalar   # etc.
    \local $escalar # etc.
    \local our $escalar # etc.
    \$un_array[$indice]
    \$un_hash{$clave}
    \local $un_array[$indice]
    \local $un_hash{$clave}
    condición ? \$este : \$ese[0]  # etc.

Operaciones sobre porciones y paréntesis hacen que la parte derecha se evalúe en contexto de lista:
...
Cada elemento en la parte derecha debe ser una referencia a un dato del tipo correcto.  Los paréntesis que rodean directamente un array (y, posiblemente también C<my>/C<state>/C<our>/C<local>) hará que cada elemento del array sea un alias al correspondiente escalar referenciado en la parte derecha:

    \(@a) = \(@b); # @a y @b ahora tienen los mismos elementos
    \my(@a) = \(@b); # lo mismo
    \(my @a) = \(@b); # lo mismo
    push @a, 3; # pero ahora @a tiene un elemento extra que a @b le falta
    \(@a) = (\$a, \$b, \$c); # @a ahora contiene $a, $b y $c

Combinando esa forma con C<local> y rodeando con paréntesis a un hash, están prohibidos (porque no está claro lo que se tendría que hacer):

    \local(@array) = foo(); # INCORRECTO
    \(%hash)       = bar(); # INCORRECTO

La asignación a referencias y no-referencias se puede combinar en listas y en expresiones del operador condicional ternario, siempre que los valores de la parte derecha sean del tipo correcto para cada elemento de la izquierda, aunque esto pueda crear código ofuscado:

    (my $tom, \my $dick, \my @harry) = (\1, \2, [1..3]);
    # $tom ahora es \1
    # $dick ahora es 2 (solo lectura)
    # @harry es (1,2,3)

    my $tipo = ref $cosita;
    ($tipo ? $type eq 'ARRAY' ? \@foo : \$bar : $baz) = $cosita;

El bucle C<foreach> también puede tomar un constructor de referencias para su variable de bucle, aunque la sintaxis se limita a una de las siguientes, con un opcional C<my>, C<state> o C<our> después de la barra diagonal inversa:
...
No se permiten paréntesis.  Esta característica es particularmente útil para los arrays-de-arrays o arrays-de-hashes:

    foreach \my @a (@array_de_arrays) {
        procesar($a[0], $a[-1]);
    }

    foreach \my %h (@array_de_hashes) {
        $h{carcajada}++ if $h{tipo} eq 'gracioso';
    }

B<ADVERTENCIA:> el crear alias no funciona correctamente con clausuras.  Si intenta crear alias a variables léxicas en el interior de una subrutina o C<eval>, solo será visible en el interior de la subrutina, y no afectará a la subrutina exterior, donde se declararon las variables.  Este comportamiento tan extraño está sujeto a cambio.
