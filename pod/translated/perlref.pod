...
La sintaxis se debe activar con C<use feature 'postderef'>.  Es experimental, y por defecto lanzará un aviso, a menos que se active C<no warnings
...
La desreferencia de array y escalar posfija se I<puede> usar en la interpolación de cadenas (comillas dobles o con el operador C<qq>), pero sólo si se activa la característica adicional C<postderef_qq>.
...
Igual que con la desreferencia posfija de array, la desreferencia posfija de porciones de valores se I<puede> usar en la interpolación de cadenas (comillas dobles o con el operador C<qq>), pero sólo si se activa la L<feature> (característica) adicional C<postderef_qq>.
...
No se permiten paréntesis.  Esta característica es particularmente útil para los arrays-de-arrays o arrays-de-hashes:
    ...
    foreach \my %h (@array_de_hashes) {
        $h{carcajada}++ if $h{tipo} == 'gracioso';
    }
