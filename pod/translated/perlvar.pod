...
Un letra Unicode que no sea ASCII, no se le considera que sea una letra a menos que esté en activo S<C<"use utf8">>, y que se aplique alguna regla más complicada; vea L<perldata/Interpretación de identificadores> para más detalles.

Los nombres de las variables Perl también pueden ser una secuencia de dígitos, un único carácter de puntuación, o una secuencia de dos caracteres: C<^> (circunflejo o CIRCUMFLEX ACCENT) seguido por uno o más caracteres C<[][A-Z^_?\]>.
Todos estos nombres están reservados para usos especiales en Perl; por ejemplo, los nombres formados exclusivamente por dígitos se usan para almacenar datos capturados por ///backreferences detectadas por una expresión regular.

Desde la versión Perl 5.6.0, los nombres de variables en Perl pueden ser cadenas alfanuméricas precedidas por un acento circunflejo.  Todas estas variables deben escribirse con la sintaxis C<${^Foo}>; las llaves no son opcionales.  C<${^Foo}> denota la variable escalar cuyo nombre se considera que es un control-C<F> seguido por dos C<o>.
Estas variables se reservan para usos especiales futuros de Perl, a excepción de los que comienzan con C<^_> (circunflejo-guión bajo).  Ningún nombre que comience con C<^_> adquirirá un significado especial en cualquier futura versión de Perl; por lo tanto, estos nombres se pueden usar con seguridad en los programas actuales.  C<$^_>, sin embargo, I<está> reservada.

Los identificadores de Perl que comienzan con dígitos o caracteres de puntuación están exentos de los efectos de la declaración del C<package> y siempre están obligados a estar en el paquete C<main>; también están exentos de los errores de C<strict 'vars'>.  Otros pocos nombres más también están exentos de esas reglas:
...
Los nombres siguientes tienen un significado especial en Perl.  La mayoría de los nombres de las variables con caracteres de puntuación tienen mnemónicos razonables, o análogos de los I<shell>s.
...
Dentro de una subrutina, el array C<@_> contiene los parámetros pasados a esa subrutina.  Dentro de una subrutina, C<@_> es el array por defecto para los operadores de array C<pop> y C<shift>.
...
El número de proceso de Perl ejecutando este I<script>.  A pesar de que I<puede> darle un valor a esta variable, por lo general se desaconseja hacerlo, aunque puede resultar muy útil para algunas comprobaciones de pruebas.  Será restablecido automáticamente a través de las llamadas C<fork()>.
...
LinuxThreads es ahora obsoleto en Linux, y cachear C<getpid()> de esta manera hace integrar perl innecesariamente complejo (debido a que tendrá que actualizar manualmente el valor de $$), así que ahora C<$$> y C<getppid()> siempre devolverán los mismos valores que la biblioteca C subyacente.

Sistemas Debian GNU/kFreeBSD también usan LinuxThreads hasta la distribución 6.0 (inclusive), pero después se trasladó a la semántica de hilos de FreeBSD, que es parecida a la POSIX.
...
Mnemónico: lo mismo que en los I<shell>.
...
En algunas plataformas puede existir una cantidad arbitraria de relleno, por ejemplo caracteres de espacio, después del nombre modificado al mostrarse por C<ps>.
...
C<< $< >>, C<< $> >>, C<$(> y C<$)> solo pueden ser establecidos en máquinas que soporten la correspondiente rutina I<set[re][ug]id()>.  C<$(> y C<$)> pueden intercambiarse en máquinas con soporte de C<setregid()>.
...
C<< $< >> y C<< $> >> pueden intercambiarse en máquinas con soporte de C<setregid()>.
...
Debido a su carácter especial, C<$a> y C<$b> no tienen que declararse (usando C<use vars>, u C<our()>) incluso cuando se utiliza el I<pragma> C<strict 'vars'>.  No las localice C<my $a> o C<my $b> si quiere utilizarlos en el bloque o función de comparación de C<sort()>.
...
Esta variable se puede usar para determinar si el intérprete de Perl que ejecuta un I<script> está en el rango correcto de versiones:
...
Vea también la documentación de C<use VERSIÓN> y C<require VERSIÓN>; allí hay una forma conveniente de cómo fallar si el intérprete de Perl en ejecución es demasiado viejo.
...
La forma C<$OLD_PERL_VERSION> se añadió a Perl v5.20.0 por razones históricas pero se desaconseja su uso. (Si su razón para usar C<$]> es el de ejecutar código en perl antiguos, entonces referirse a él como C<$OLD_PERL_VERSION> será como una derrota auto infligida).
...
La política predeterminada de entrega de señales cambió en Perl v5.8.0, de "inmediato" (también conocido como "inseguro") a "aplazado", también conocido como "señales seguras".  Vea L<perlipc> para más información.
...
La rutina indicada por C<$SIG{__DIE__}> se llama cuando una excepción grave está a punto de lanzarse.  El mensaje de error se pasa como primer argumento.  Cuando una rutina enganchada en C<__DIE__> regresa, el procesamiento de la excepción continúa como si no existiera el enganche, a menos que la rutina enganchada termine a través de un C<goto &sub>, la salida de un bucle, o un C<die()>.  El gestor de C<__DIE__> está explícitamente inhabilitado durante la llamada, para que pueda morir desde un controlador C<__DIE__>.  Lo mismo para C<__WARN__>.

Debido a un fallo de implementación, el enganche C<$SIG{__DIE__}> se llama incluso dentro de un C<eval()>.  No lo utilice para volver a reescribir una excepción pendiente en C<$@>, o como un sustituto extraño para sobreescribir C<CORE::GLOBAL::die()>.  Esta acción extraña se arreglará en una futura distribución de manera que C<$SIG{__DIE__}> se llamará solo si el programa está a punto de terminar, como era la intención original.  Cualquier otro uso es obsoleto.
...
Consulte la documentación de C<use VERSIÓN> y C<require VERSIÓN> para ver una forma correcta de cómo fallar si el intérprete de Perl es demasiado viejo.
...
Pero recuerde que no todos los sistemas operativos son compatibles con fork() o con la captura de la salida de los comandos, por lo que esta compleja declaración quizás no sea portable.
...
Debido a que muchos sistemas operativos permiten a cualquier persona con acceso de lectura al archivo de programa en Perl para hacer una copia de él, parchear la copia y, a continuación, ejecutar la copia, el programador de Perl consciente de la seguridad debe tener cuidado de invocar la copia instalada de perl, no la copia referenciada por C<$^X>.  Las siguientes declaraciones logran este objetivo, y producen una ruta que puede invocarse como un comando o referenciada como un archivo.
...
El comportamiento subyacente de C<%+> se proporciona por el módulo L<Tie::Hash::NamedCapture>.
...
El array C<@ARGV> contiene los argumentos de la línea de comandos que se pasaron al I<script>.  C<$#ARGV> es, generalmente, el número de argumentos menos uno, debido a que C<$ARGV[0]> es el primer argumento, I<no> es el nombre del propio programa.  Vea L</$0> para obtener el nombre del comando.
...
El estado nativo devuelto por la última tubería cerrada, acento grave (C<``>) de comandos, llamada con éxito a C<wait()> o C<waitpid()>, o del operador C<system()>.  En sistemas tipo POSIX este valor puede decodificarse con las funciones WEXITSTATUS, WIFEXITED, WIFSIGNALED, WTERMSIG, WIFSTOPPED, WSTOPSIG, y WIFCONTINUED proporcionadas por el módulo L<POSIX>.

Bajo VMS esto refleja el estado real de la salida VMS; es decir, es lo mismo que C<$?> cuando está en vigor el I<pragma> C<use vmsish 'status'>.
...
Información específica de error del actual sistema operativo.  Por el momento, esta se diferencia de C<L</$!>> solo bajo VMS, OS/2 y Win32 (y para MacPerl).  En todas las demás plataformas, C<$^E> es siempre lo mismo que C<$!>.
...
Estado actual del intérprete.
	...
	$^S         Estado
	---------   -------------------------------------
	undef            Interpretar un módulo, eval, o programa principal
	verdadero (1)    Ejecutar un eval()
...
El conjunto actual de los controles de advertencia activados por el I<pragma> C<use warnings>.
Tiene el mismo alcance léxico que las variables C<$^H> y C<%^H>.  Los valores exactos se consideran internos al I<pragma> L<warnings> y puede cambiar entre versiones de Perl.
...
Si se le asigna un valor numérico, el valor se almacena en C<errno>.
...
Cada elemento de C<%!> tiene un valor verdadero sólo si C<$!> se ajusta a ese valor.  Por ejemplo, C<$!{ENOENT}> es verdadero si y sólo si el valor actual de C<$!> es C<ENOENT>, es decir, si el error más reciente fue "No existe el archivo o directorio" (o su equivalente moral: no todos los sistemas operativos dan exactamente este error, y ciertamente, no en todos los idiomas).  El verdadero valor específico no se garantiza, pero en el pasado ha sido, generalmente, el valor numérico de C<$!>.  Para comprobar si una clave particular es significativa en su sistema, utilice C<exists $!{la_clave}>; para obtener una lista de las claves legales, utilice C<keys %!>.  Vea L<Errno> para más información, y también vea L</$!>.
...
Bajo VMS, el I<pragma> C<use vmsish 'status'> hace que C<$?> refleje el estado real de la salida VMS, en lugar de la emulación predeterminada del estado POSIX; vea L<perlvms/$?> para más detalles.
...
El mensaje de error de Perl en el último operador C<eval>, es decir, la última excepción que se cazó.  Para un C<eval BLOQUE>, este puede ser tanto un mensaje de error en tiempo de ejecución o la cadena o el C<die> que se llamó.  La forma C<eval CADENA> también captura errores de sintaxis y otras excepciones en tiempo de compilación.

Si no ocurre un error, C<eval> establece C<$@> a la cadena vacía.

Los mensajes de advertencia no se almacenan en esta variable.  Puede, sin embargo, establecer una rutina para procesar las advertencias estableciendo C<$SIG{__WARN__}> tal como se describe en L</%SIG>.

Mnemónico: ¿"En" dónde se encontraba el error?
...
Su uso principal es con B<-MO=...> para permitir al código alterar su comportamiento cuando se compila, tal como por ejemplo hace C<AUTOLOAD> en tiempo de compilación, en lugar de la más normal carga aplazada.  Haciendo C<$^C = 1> es similar a llamar a C<B::minus_c>.
...
El valor actual de los indicadores de depuración.  Puede leerse o modificarse.  Como su equivalente de la L<línea de comandos|perlrun/B<-D>I<letras>>, puede usar los valores numéricos o los simbólicos, p. ej. C<$^D = 10> o C<$^D = "st">.  Consulte L<perlrun/B<-D>I<números>>.  Los contenidos de esta variable también afecta a la operación del depurador.  Vea L<perldebguts/Interioridades del depurador>.
...
La I<referencia de objecto> al objeto C<Encode> que se ha utilizado para convertir el código fuente a Unicode.  Gracias a este variable su I<script> Perl no tiene por qué estar escrito en UTF-8.  El valor predeterminado C<undef>.
...
Si necesita algo como esta funcionalidad, debe utilizar el I<pragma> L<encoding>, que también es obsoleto, pero con unos pocos efectos desagradables.
...
El C<PerlInterpreter*> se ha construido a través de C<perl_construct>.  Este valor está aquí principalmente para complementar y para uso a través de la variable subyacente de C C<PL_phase>.  No es realmente posible ejecutar el código Perl a no ser que la construcción del intérprete esté terminada.
...
Este comportamiento proporciona la semántica del ámbito léxico, y se utiliza en, por ejemplo, el I<pragma> C<use strict>.
...
demuestra cómo está implementado C<use strict 'vars'>.  Aquí hay una versión condicional del mismo I<pragma> léxico:
...
Depuración línea a línea.  Hace que la subrutina C<DB::DB()> se llame por cada instrucción ejecutada.  También hace que se almacenen las líneas del código fuente (como 0x400).
...
C<$*> era una variable que podría utilizarse para habilitar concordancia multilínea.
...
Con la distribución de Perl 5, las asignaciones a C<$[> se trataban como directivas de compilación, y no podían influir en el comportamiento de cualquier otro archivo.
