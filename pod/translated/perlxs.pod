...
El compilador XS se llama B<xsubpp>.  Este compilador crea las construcciones necesarias para permitirle a un XSUB que manipule valores Perl, y crea el pegamento necesario para permitirle a Perl llamar a XSUB.  El compilador usa B<typemaps> para determinar cómo mapear parámetros de función C y valores de salida a valores Perl, y al revés.  El mapa de tipos por defecto (que viene con Perl) gestiona muchos tipos comunes de C.  Un mapa de tipos suplementario también puede ser necesario para gestionar cualesquiera estructuras y tipos especiales y para la biblioteca enlazada. Para más información sobre typemaps, vea L<perlxstypemap>.

Un archivo en formato XS empieza con una sección en lenguaje C que va hasta la primera directiva C<MODULE =Z<>>.  Otras directivas XS y definiciones XSUB pueden seguir a esta línea.  EL "lenguaje" usado en esta parte del archivo es normalmente se conoce como lenguaje XS.  B<xsubpp> reconoce y se salta POD (vea L<perlpod>) tanto en C como en secciones del lenguaje XS, que permite al archivo XS contener documentación integrada.
...
A partir de Perl 5.16, puede integrar mapas de tipos en su contigo XS en lugar de o añadido al mapa de tipos en un archivo separado.  Múltiples de tales mapas de tipos integrados se procesarán en orden de aparición en el código XS e igual a los archivos locales de mapas de tipos toman precedencia sobre el mapa de tipos por defecto; los mapas de tipos integrados pueden sobrescribir las definiciones anteriores de estancias de TYPEMAP, INPUT y OUTPUT.  La sintaxis para los mapas de tipos integrados es
...
La palabra clave PROTOTYPES: corresponde a las opciones C<-prototypes> y C<-noprototypes> de B<xsubpp>.  Esta palabra clave reescribe las opciones de la línea de comandos.
