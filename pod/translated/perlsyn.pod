...
En la implementación actual, el bucle C<foreach> puede estar en cualquier lugar dentro del ámbito dinámico del modificador C<when>, pero debe estar dentro del ámbito léxico del bloque C<given>.  Esta restricción puede relajarse en una futura versión.  Vea a continuación L</"Instrucciones Switch">.
...
Si existe un BLOQUE C<continue>, siempre es ejecutado justo antes de que la condición se evalúe de nuevo.  Así, puede usarse para incrementar una variable de bucle, incluso cuando el ciclo se ha parado por medio de la instrucción C<next>.
