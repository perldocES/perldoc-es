...
Lo más importante es ejecutar siempre los programas con la opción B<-w>.  Puede desactivarla explícitamente en partes del código con el pragma C<no warnings> o con la variable C<$^W>, si así lo desea.  También debe ejecutar siempre los programas con C<use strict>; si no lo hace, debe saber por qué no necesita hacerlo.  El pragma C<use sigtrap> y C<use diagnostics> pueden ser también muy útiles.
...
Evite usar C<grep()> (o C<map()>) o `acentos graves` en un contexto nulo (es decir, desechando los valores de retorno).  Estas funciones devuelven valores, por lo que debe usarlos.  Como alternativa, use un bucle C<foreach()> o la función C<system()>.
