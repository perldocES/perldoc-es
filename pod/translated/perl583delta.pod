...
Este documento describe las diferencias entre las distribuciones 5.8.2 y 5.8.3.

Si está actualizando desde una distribución anterior como la 5.6.1, primero lea L<perl58delta>, que describe las diferencias entre 5.6.0 y 5.8.0, y L<perl581delta> y L<perl582delta>, que describe las diferencias entre 5.8.0, 5.8.1 y 5.8.2.
...
La documentación se ha revisado en diversos lugares para producir páginas de manual más estándares.

La documentación de los bloques especiales (BEGIN, CHECK, INIT, FIN) se ha mejorado.
...
join() podría devolver basura cuando la misma instrucción join() fue anteriormente usada para procesar datos de 8 bits habiendo procesado anteriormente datos UTF8, debido a que los indicadores en esa área de trabajo temporal no eran correctamente reinicializadas. Esto se ha arreglado.
...
El código hash se ha reprogramado para reducir la duplicación de código fuente. El interfaz externo no se ha modificado, y aparte de la corrección de errores descritos anteriormente, no debería haber ningún cambio en el comportamiento.
...
Hay una conocida condición de carrera al abrir I<scripts> con C<suidperl>. C<suidperl> no se construye ni instala por defecto, y es obsoleto desde perl 5.8.0. Se recomienda reemplazar el uso de suidperl con herramientas como sudo (http://www.courtesan.com/sudo/)
...
Código congelado para la próxima distribución de mantenimiento (5.8.4) será el 31 de marzo de 2004, con la distribución prevista para mediados de abril. Del mismo modo, la congelación de 5.8.5 será a finales de junio, con la distribución a mediados de julio.
