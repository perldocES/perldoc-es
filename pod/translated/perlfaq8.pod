...
Note que I<debe> usar la sintaxis de redirección con comillas inversas del I<shell> Bourne (C<sh(1)>), ¡no de la C<csh(1)>!  Detalles sobre por qué C<system()> de Perl y las comillas inversas y las aperturas de tuberías usan el I<shell> Bourne están en el artículo F<versus/csh.whynot> en la colección "I<Far More Than You Ever Wanted To Know>" en L<http://www.cpan.org/misc/olddoc/FMTEYEWTK.tgz> . Para capturar el STDOUT y STDERR del comando, juntos:
...
C<system> también proporciona control directo sobre si toma lugar el procesado de los comodines del I<shell>, mientras que eso no ocurre con las comillas inversas.

=head2 ¿Cómo puedo llamar a las comillas inversas sin que lo procese el I<shell>?
