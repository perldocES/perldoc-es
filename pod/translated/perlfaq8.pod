...
La variable C<$^O> (C<$OSNAME> si usa C<English>) contiene una indicación del nombre del sistema operativo (no su número de distribución) donde se compiló su binario perl.
...
Si tiene un programa en línea de comandos que hace el trabajo, puede llamarlo con las comillas invertidas para capturar cualquier salida suya, así que puede usarlo más tarde:
...
También puede hacer esto en la mayor parte de los sistemas usando el módulo L<Term::ReadKey> de CPAN, que es más fácil de usar y en teoría más portable.
...
=head2 ¿Cómo decodifico archivos de contraseñas cifradas?

Usted se gasta montones y montones de dinero en hardware dedicado, pero es de esto de lo que realmente quiere hablar.

En serio, no puede si se trata de archivos de contraseñas Unix. El sistema de contraseñas Unix emplea cifrado de un único sentido. Es más como un I<hashing> que un cifrado. Lo mejor que puede hacer es comprobar si otra cosa se cifra a la misma cadena. No puede obtener la cadena original a partir de un hash. Programas como Crack puede, por la fuerza (e inteligentemente) intentar adivinar las contraseñas, pero no garantiza (no puede) un éxito rápido.
...
=head2 ¿Por qué mi programa con I<sockets> no funciona en System V (Solaris)? ¿Qué significa el mensaje de error "I<Protocol not supported>" (Protocolo no soportado)?

Algunos sistemas basados en Sys-V, de forma notable Solaris 2.X, redefinieron algunas de las constantes estándares de los I<sockets>. Ya que eran constantes en todas las arquitecturas, se fijaban al código perl. La forma adecuada de tratar esto es con "use Socket" para obtener los valores correctos.
...
Está confundiendo el propósito de C<system()> y las comillas invertidas (``). C<system()> ejecuta un comando y devuelve la información del estado de la salida (como un valor de 16 bit: los 7 bit más bajos son la señal que provocó que el proceso muriese, si fuere el caso, y los 8 bit más altos es el valor real de la salida). Las comillas invertidas (``) ejecutan un comando y devuelven lo que se envía a STDOUT.
...
Existen tres formas básicas de ejecutar comandos externos:
    ...
    my $output = `$cmd`;        # utilizando comillas invertidas (``)
...
Comillas invertidas y C<open()> leen B<solo> el STDOUT de su comando.
...
o puede usar la redirección de descriptores de archivo del I<shell> Bourne:
...
Note que I<no puede>, simplemente, abrir STDERR para que sea un duplicado de STDOUT en su programa Perl y evitar el llamar al I<shell> para hacer la redirección.
Esto no funciona:

    open(STDERR, ">&STDOUT");
    $todalasalida = `cmd args`;  # stderr se sigue escapando

Esto falla porque el C<open()> hace que STDERR vaya a donde STDOUT está yendo en el momento del C<open()>. Las comillas invertidas hacen que STDOUT vaya a una cadena, pero no cambia STDERR (que sigue yendo al anterior STDOUT).

Note que I<debe> usar la sintaxis de redirección con comillas invertidas del I<shell> Bourne (C<sh(1)>), ¡no de la C<csh(1)>!  Detalles sobre por qué C<system()> de Perl y las comillas inversas y las aperturas de tuberías usan el I<shell> Bourne están en el artículo F<versus/csh.whynot> en la colección "I<Far More Than You Ever Wanted To Know>" en L<http://www.cpan.org/misc/olddoc/FMTEYEWTK.tgz> . Para capturar el STDOUT y STDERR del comando, juntos:

    $salida = `cmd 2>&1`;                       # tanto con comillas invertidas
    $pid = open(PH, "cmd 2>&1 |");              # como con una apertura de una tubería
    while (<PH>) { }                            #    y leer

Para capturar el STDOUT de un comando pero descartar su STDERR:

    $salida = `cmd 2>/dev/null`;                # tanto con comillas invertidas
...
Para capturar el STDERR de un comando pero descartar su STDOUT:
    ...
    $salida = `cmd 2>&1 1>/dev/null`;           # tanto con comillas invertidas
...
Intercambiar el STDOUT y STDERR de un comando para capturar el STDERR pero dejando que su STDOUT salga por el anterior STDERR:
    ...
    $salida = `cmd 3>&1 1>&2 2>&3 3>&-`;        # tanto con comillas invertidas
...
El orden es importante en todos estos ejemplos. Esto es porque el I<shell> procesa las redirecciones de descriptores de archivo en un estricto orden de izquierda a derecha.
...
Si el segundo argumento a un C<open()> entubado contiene metacaracteres I<shell>, perl se C<fork()>ea y luego C<exec()>uta un I<shell> para decodificar los metacaracteres, y finalmente ejecutar el programa deseado. Si el programa no se pudiera ejecutar, es el I<shell> el que obtiene el mensaje, no Perl. A lo largo de su programa Perl se puede saber si el I<shell> se ejecutó de forma exitosa. Puede aún seguir capturando el STDERR del I<shell> y ver si contiene mensajes de error. Vea L<"¿Cómo puedo capturar STDERR desde un comando externo?"> en este documento, o use el módulo L<IPC::Open3>.

Si no existen metacaracteres I<shell> en el argumento de C<open()>, Perl ejecuta el comando directamente, sin usar el I<shell>, y puede informar correctamente si el comando se inició.

=head2 ¿Qué tiene de malo usar las comillas invertidas en un contexto vacío?

Estrictamente hablando, nada. Estilísticamente hablando, no es una buena manera de escribir código mantenible. Perl tiene diversos operadores para ejecutar comandos externos. Las comillas invertidas son uno de ellos; recolectan la salida del comando para usarse en su programa. La función C<system> es otro; no hace eso.

Escribir comillas invertidas en su programa manda un claro mensaje a los lectores de su código de que quiere recolectar la salida del comando.
...
C<system> también proporciona control directo sobre si toma lugar el procesado de los comodines del I<shell>, mientras que eso no ocurre con las comillas invertidas.

=head2 ¿Cómo puedo llamar a las comillas invertidas sin que lo procese el I<shell>?
...
Igual que las formas de lista de C<system()> y C<exec()>, no ocurre el escapado del I<shell>.
...
Igual que con C<system()>, no ocurre el escapado I<shell> cuando C<exec()>uta una lista. Más ejemplos de esto se pueden encontrar en L<perlipc/"Open entubados seguros">.
...
=head2 ¿Cómo puedo convertir mi I<script> en I<shell> a perl?

Aprenda Perl y reescríbalo. En serio, no hay un conversor sencillo.
Cosas que son incómodas hacer en el I<shell> son fáciles de hacer en Perl, y esta incomodidad es lo que podría hacer imposible escribir un conversor de I<shell> a perl. Reescribiéndolo, pensará en lo que realmente está intentando hacer, y posiblemente escapará el paradigma de entubado del I<shell>, que mientras es conveniente para algunos asuntos, puede causar muchas ineficiencias.
...
Para alterar realmente la parte visible de la línea de comandos, puede asignar a la variable $0 como se documenta en L<perlvar>. Aún así, esto no funcionará en todos los sistemas operativos. Programas demonio como sendmail modifican su estado, así:
...
En un sentido estricto, no se puede hacer: el I<script> se ejecuta como un proceso diferente desde el I<shell> desde donde se arrancó. Los cambios no se reflejan en su padre; solo en los hijos creados después del cambio. Existe una magia en el I<shell> que le puede permitir falsificarlo C<eval()>uando la salida del I<script>; mire el documento P+F de comp.unix.questions por los detalles.
...
Después de 10 segundos de tiempo consumido en la CPU (no tiempo "mural"), el proceso enviará una señal (XCPU en algunos sistemas) que, si no es capturada, causará que el proceso termine. Si la señal es capturada, entonces después de otros 10 segundos más (20 segundos en total) el proceso se matará con una señal no atrapable.
...
=head2 ¿Cómo distingo entre errores del I<shell> y los de perl?
...
Obtengo un error cuando ejecuto esto desde mi I<shell> (que sucede que es bash). Puede parece que perl olvidó que tiene una función C<print()>, pero mi línea I<shebang> no es la ruta a perl, así que la I<shell> ejecuta el I<script> y obtiene el error.
...
Podría también saberse todos los errores perl, y aunque algunas personas quizás se lo sepan, es probable que usted no. Aun así, deberían estar en la página de manual L<perldiag>. Si allí no encuentra el error, probablemente no es un error perl.
...
Puede establecerlo en su configuración de C<CPAN.pm> para que así los módulos se instalen automáticamente en su directorio privado de bibliotecas cuando use el I<shell> de CPAN.pm:
...
La última es particularmente útil porque sabe distinguir entre diferentes arquitecturas. El módulo pragmático C<lib.pm> se incluyó por primera vez con la distribución 5.002 de Perl.
