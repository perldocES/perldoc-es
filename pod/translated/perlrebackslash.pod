...
El conjunto exacto de caracteres que coinciden con C<\d>, C<\s> y C<\w> varía dependiendo de varios I<pragma> y modificadores de expresiones regulares.  Es posible restringir la coincidencia al rango ASCII usando el modificador de expresiones regulares C</a>.  Vea L<perlrecharclass>.
...
Todas las determinaciones simples de límite, C<\b> y C<\B>, buscan por caracteres de palabra sueltos, no por caracteres de no palabra, ni por finales de cadena.  Puede ayudar a entender cómo funcionan C<\b> y C<\B> equiparándolos a lo que sigue:

    \b	significa realmente	(?:(?<=\w)(?!\w)|(?<!\w)(?=\w))
    \B	significa realmente	(?:(?<=\w)(?=\w)|(?<!\w)(?!\w))

En contraste, C<\b{...}> y C<\B{...}> pueden o no coincidir con el comienzo y el final de la línea, dependiendo del tipo de límite.  Estas implementan los límites por defecto de Unicode, especificados en L<http://www.unicode.org/reports/tr14/> y L<http://www.unicode.org/reports/tr29/>.
Los tipos de límite son:
...
Esto coincide de acuerdo a lo que indica el valor por defecto de I<Unicode Line Breaking Algorithm> (L<http://www.unicode.org/reports/tr14/>), tal y como está personalizado en ese documento (L<Example 7 of revision 35|http://www.unicode.org/reports/tr14/tr14-35.html#Example7>) para un mejor manejo de las expresiones numéricas.

Esto sirve para varios propósitos, pero el módulo L<Unicode::LineBreak> está disponible en CPAN, que ofrece muchas más características, incluyendo la personalización.

=item C<\b{sb}>

Coincide con un "I<Sentence Boundary>" (Límite de frase) de Unicode.  Esto es una ayuda para interpretar frases en lenguaje natural.  Da buenos resultados, pero imperfectos.  Por ejemplo, cree que "Sr. Pérez" son dos frases.  Hay más detalles en L<http://www.unicode.org/reports/tr29/>.  Note también que él cree que cualquier coincidencia con L</\R> (excepto con el avance de página y el tabulador vertical) es un límite de frase.  C<\b{sb}> funciona con texto diseñado para procesadores de texto que desplazan líneas de forma automática, al mostrarlas, pero los límites incluidos en el propio texto se considerarán esencialmente como los finales de los bloques de texto (párrafos, realmente), y de ahí, los finales de frase.  C<\b{sb}> no lo hace bien con texto que contenga caracteres de nueva línea integrados, como el texto del documento que está leyendo ahora.
...
Coincide con un "I<Word Boundary>" (Límite de palabra) de Unicode, pero ajustado a las expectativas de Perl.  Ofrece un mejor resultado (aunque no perfecto) para el procesado de lenguaje natural que el simple C<\b> (sin llaves).  Por ejemplo, entiende que los apóstrofes pueden estar en medio de palabras y que los paréntesis, no (vea los ejemplos de más abajo).  Hay más detalles en L<http://www.unicode.org/reports/tr29/>.

La definición actual de Unicode de un límite de palabra coincide entre todos los caracteres de espacio en blanco.  Perl ajusta esto, comenzando en la versión 5.24, para, de forma general, no romper conjuntos de espacios en blanco, tal y como siempre funcionaba un sencillo C<\b>.  Esto permite que C<\b{wb}> sea un reemplazo de C<\b>, pero, generalmente, con mejores resultados para el procesado de lenguaje natural.  (La excepción a este ajuste es cuando a un conjunto de espacios en blanco se le sigue algo como U+0303, COMBINING TILDE.
Si el carácter espacio final en el conjunto, es un espacio en blanco horizontal, lo rompe, para de esta manera unirse en lugar de combinar un carácter.
Para ser preciso, si un conjunto de espacio en blanco que acaba en un espacio horizontal, se le sigue inmediatamente un carácter, que tenga tanto los valores de propiedad de límite "Extendido" (I<Extend>) o "Formato" (I<Format>), el límite entre el carácter de espacio horizontal y el resto del conjunto coincide con C<\b{wb}>.  En todos los otros casos, el límite entre dos caracteres espacios en blanco coincide con C<\B{wb}>).
