...
Es posible establecer C<@ISA> manualmente; puede ver esto en código Perl antiguo. En el código realmente antiguo también se usaba el I<pragma> L<base>. Pero en el código nuevo recomendamos usar el I<pragma> L<parent> para declarar las clases primarias.
Este I<pragma> se ocupará de establecer C<@ISA>. También cargará las clases primarias y se asegurará de que el paquete no herede de sí mismo.
...
Ya hemos hablado sobre el array especial C<@ISA> y el I<pragma> L<parent>.
...
Se puede usar el I<pragma> L<mro> para solicitar un orden de resolución de métodos distinto.
...
El I<pragma> L<mro> proporciona algunas funciones para manipular directamente la memoria caché de métodos.
...
De forma análoga, la función predefinida C<ref> trata de forma especial las referencias a objetos bendecidos. Si llama a C<ref($algo)> y C<$algo> contiene una referencia a un objeto, devolverá el nombre de la clase en la que se bendijo el objeto.
...
Si se usa el I<pragma> L<strict> sin la declaración C<our $AUTOLOAD>, este código no se compilará.
...
Como los métodos C<DESTROY> pueden llamarse en cualquier momento, debe localizar cualquier variable global que pueda actualizarse mediante C<DESTROY>. En particular, si usa C<eval {}> debería localizar C<$@>, y si usa C<system> o comillas inversas, debería localizar C<$?>.
