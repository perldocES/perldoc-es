...
Este documento describe las diferencias entre las distribuciones 5.10.0 y 5.10.1.

Si está actualizando desde una distribución anterior como la 5.8.8, primero lea L<perl5100delta>, que describe las diferencias entre 5.8.8 y la 5.10.0.

=head1 Cambios incompatibles

=head2 Cambios en la instrucción Switch

El manejo de expresiones complejas por C<given>/C<when> de la instrucción switch se ha mejorado. Hay dos nuevos casos en el que ahora C<when> interpreta su argumento como un valor booleano, en lugar de una expresión que se utiliza en una coincidencia inteligente:
...
Ya no se tratan de forma especial las referencias a código con un prototipo vacío.
...
Debido a la rotura de la conmutatividad, las referencias a código ya no se tratan de forma especial cuando aparecen a la izquierda del operador C<~~>, sino que se tratan como cualquier otro valor escalar.
...
Vea L<"Módulos y pragmas"> para más información.
...
Como parte de la actualización de C<ExtUtils::MakeMaker>, los módulos C<ExtUtils::MakeMaker::bytes> y C<ExtUtils::MakeMaker::vmsish> se han eliminado de esta distribución.
...
La copia de la base de datos de caracteres Unicode incluidos en Perl 5.10.1 se ha actualizado desde la 5.0.0 a la 5.1.0. Vea L<http://www.unicode.org/versions/Unicode5.1.0/#Notable_Changes> para consultar los cambios principales.
...
=head2 El I<pragma> C<overloading>

Este I<pragma> le permite desactivar o activar localmente la sobrecarga de algunas o todas las operaciones. (Yuval Kogman)
...
Tenga en cuenta que en la actualidad algunos I<scripts> de prueba pueden fallar cuando se ejecuta en paralelo (sobre todo C<ext/IO/t/io_dir.t>). Si es necesario ejecute los I<scripts> una vez más de forma secuencial y mire a ver si los errores desaparecen.
...
La versión incluida es la 2.06_01. Tenga en cuenta que en esta distribución, usar una cadena C<eval>uada cuando C<autodie> está activo puede causar que el comportamiento de autodie se fugue del ámbito actual. Vea L<autodie/"ERRORES"> para más detalles.
...
Este I<pragma> establece una relación ISA con clases base en tiempo de compilación. Proporciona la característica clave de C<base> sin la función de fluencia.
...
Esto se basa en el supuesto de que las nuevas funciones, en general, no se añadirá a las distribuciones de mantenimiento. Por tanto, C<:5,10> y C<:5.10.X> tienen un efecto idéntico. Esto es un cambio en el comportamiento documentado para 5.10.0.
...
Vea más arriba L</"El I<pragma> C<overloading>">.
...
Note que C<ExtUtils::MakeMaker::bytes> y C<ExtUtils::MakeMaker::vmsish> se han eliminado de esta distribución.
...
Actualizado desde la versión 1.05 a la 2.06_01. Vea también el nuevo I<pragma> C<autodie>.
...
Actualizado desde la versión 2.13 a la 2.18. Esta distribución ya no contiene el hash C<Module::CoreList::patchlevel>.
...
Tenga en cuenta que un efecto secundario de la 2.x para actualizar a la 3.x es que el módulo experimental C<Test::Harness::Straps> (y sus módulos de apoyo C<Assert>, C<Iterator>, C<Point> y C<Results>) se ha eliminado. Si todavía lo necesita, están disponibles en la distribución (no mantenida) C<Test-Harness-Straps> en CPAN.
...
Los grandes archivos C<Changes*> (que listaban todos los cambios realizados a perl en los últimos 18 años) se han eliminado y reemplazado por un pequeño archivo, también llamado C<Changes>, que sólo explica cómo esa misma información puede extraerse del sistema de control de versiones git.

El archivo F<Porting/patching.pod> se ha eliminado, ya que principalmente describe la interacción con el antiguo repositorio basado en Perforce, que ya está obsoleto.
...
La disposición de los directorios en F<ext> se ha revisado. En concreto, todas las extensiones ahora son planas, y en el nivel superior, con la C</> en las rutas reemplazada por C<->, de modo que F<ext/Data/Dumper/> ahora es F<ext/Data-Dumper/>, etc. Los nombres de las extensiones, como se especifica en F<Configure>, y según lo informado por C<%Config::Config>, bajo las claves C<dynamic_ext>, C<known_extensions>, C<nonxs_ext> y C<static_ext> no han cambiado, y siguen utilizando C</>. Por lo tanto, este cambio no tendrá ningún efecto una vez se haya instalado perl. Sin embargo, C<Attribute::Handlers>, C<Safe> y C<mro> se han convertido en extensiones por derecho propio, por lo que si se ejecuta F<Configure> con opciones para especificar una lista exacta de las extensiones a generarse, tendrá que cambiarla para tenerlo en cuenta.

Por 5.10.2, se prevé que muchos módulos con una doble vida se muevan desde F<lib> a F<ext>; de nuevo, esto no tendrá ningún efecto en un perl ya instalado, pero importará si llama a F<Configure> con una lista de extensiones pre-enlatadas para generarse.

=head2 Mejoras en la configuración

Si C<vendorlib> y C<vendorarch> son iguales, entonces sólo se agregan a C<@INC> una vez.

C<$Config{usedevel}> y el C<PERL_USE_DEVEL> de nivel C se definen ahora si perl se compila con C<-Dusedevel>.
...
En Unix, si se genera a partir de un árbol conteniendo un repositorio git, el proceso de configuración anotará el hash de confirmación que haya obtenido, para mostrarlo en la salida de C<perl -v> y C<perl -V>. Las confirmaciones no almacenadas son automáticamente agregadas a la lista de parches locales mostrada por C<perl -V>.
...
Eliminado F<libgdbm> de AIX 5L y 6.1. El F<libgdbm> se entrega como un paquete opcional con la caja de herramientas AIX. Por desgracia, la versión de 64 bits está rota.

Cambios adecuados significa que AIX 4.2 debería funcionar de nuevo.

=item Cygwin

En Cygwin ahora quitamos el último número de la DLL. Este ha sido el comportamiento en la generación de cygwin.com, desde hace años. Los archivos de pistas se han actualizado.

=item FreeBSD

Los archivos de pistas ahora identifican las bibliotecas correctas de hilado en FreeBSD 7 y versiones posteriores.
...
Los parches de los responsables de Haiku se han fusionado. Perl ahora debería generarse en Haiku.
...
Varios cambios desde Stratus se han mezclado.
...
Un mejor manejo de ventana de mensajes significa que los mensajes C<alarm> y C<kill> ya no se desecharán cuando ocurran condiciones de carrera.
...
Habilitando el nombre lógico C<PERL_VMS_POSIX_EXIT> se codifica un estado de salida POSIX en un valor de condición VMS para una mejor interacción con el I<shell> bash de GNV y otras utilerías que dependen de los valores de salida de POSIX.  Vea L<perlvms/"$?"> para los detalles.
...
5.10.0, inadvertidamente, desactivaba una optimización, lo que provocó una caída de rendimiento apreciable en la asignación de listas, que a menudo se utiliza para asignar parámetros de función a C<@_>. La optimización se ha reinstalado, y se arregló la regresión de rendimiento.
...
Varios errores en el nuevo código mro en 5.10.0, activados al manipular C<@ISA>, se han encontrado y arreglado.
...
Dentro de archivos de código fuente Perl codificados en UTF8 (es decir, cuando C<use utf8> está en vigor), las cadenas literales entrecomilladas dobles podrían resultar dañadas cuando un C<\xNN>, C<\0NNN> o C<\N{}> se sigue por un carácter literal con un valor ordinal superior a 255 [RT # 59908].
...
C<length> en un escalar atado que devolvía un valor Unicode, podría no ser correcto, en una primera vez. Esto se ha corregido.
...
Una condición de carrera dentro de C<PerlIOStdio_close()> se ha identificado y solucionado. Esto solía provocar diversos problemas de subprocesamiento, incluyendo SEGV.

=item *

En C<unpack>, el uso de grupos C<()> en contexto escalar, hacían que fueran internamente colocados en una lista en la pila del intérprete, que se manifiesta de varias maneras, incluyendo SEGV.  Esto se ha arreglado [RT #50256].
...
En la distribución 5.10.0, C<inc_version_list> podría, incorrectamente, listar C<5.10.*> después de C<5.8.*>; esto afectó al orden de búsqueda de C<@INC> [RT #67628].
...
En la distribución 5.10.0, un método C<AUTOLOAD> creado dinámicamente podría ser pasado por alto (problema con el caché de métodos) [RT #60220,60232].

=item *

En la distribución 5.10.0, una combinación de C<use feature> y C<//ee> podría causar una fuga de memoria [RT #63110].
...
Esta advertencia se ha eliminado. En general, sólo se generaba en conjunto con otras advertencias, y su eliminación permitía añadir una optimización de búsqueda de ISA.

=item C<v-string in use/require is non-portable>

Esta advertencia se ha eliminado.
...
Las citas de J.R.R. Tolkien al principio del archivo fuente en C se han verificado y añadido referencias apropiadas, gracias a un parche de Tom Christiansen.
...
Ahora se exportan las funciones C<PerlIO_find_layer> y C<PerlIO_list_alloc>.

=item *

C<PL_na> se ha exterminado del código principal, reemplazado por STRLEN locales temporales, o llamadas C<*_nolen()>. Cualquiera de estos enfoques es más rápido que C<PL_na>, que es una desreferencia de un puntero en la estructura del intérprete bajo ithreads, y una variable global en caso contrario.
...
Una nueva herramienta, F<Porting/expand-macro.pl> ha sido añadida, que permite ver cómo una macro del preprocesador C será ampliada cuando se compile.
...
Pruebas de la interacción de C<index> e hilos.

=item t/op/pat_thr.t

Pruebas de la interacción de patrones e hilos esotéricos.
...
Pruebas de la interacción de expresiones regulares e hilos.

=item t/op/regexp_qr_embed_thr.t

Pruebas de la interacción de patrones con C<qr//> integrados e hilos.
...
Pruebas de la interacción de las propiedades Unicode e hilos.
...
Pruebas de la interacción de C<substr> e hilos.
...
El I<pragma> C<charnames> puede generar un error en tiempo de ejecución cuando una expresión regular es interpolada [RT #56444]:
...
C<Switch> es defectuoso y debe evitarse. De Perl 5.11.0 en adelante, se pretende que el uso de la versión principal de este módulo emita una advertencia, y que el módulo, con el tiempo, se elimine del núcleo principal (probablemente en perl 5.14.0). Vea L<perlsyn/"Instrucciones Switch"> para ver sus reemplazos.

=item *

C<suidperl> se eliminará en 5.12.0. Esto proporciona un mecanismo para emular los bits de permiso setuid en sistemas que no lo soportan correctamente.

=back

=head1 Agradecimientos

Parte del trabajo realizado en esta distribución se ha financiado con una ayuda de la TPF.

Nicholas Clark se retiró oficialmente del mantenimiento calabacero a finales de 2008, pero en realidad él ha puesto mucho esfuerzo desde entonces para ayudar a 5.10.1 a llegar a un estado apto para publicarse, incluyendo la redacción de una parte considerable de este perldelta.
...
El otro grupo principal de contribuidores contribuyó con la mayor parte de los cambios, y aplicaron la mayoría de los parches enviados por cientos de colaboradores que figuran en F<AUTORES>.
