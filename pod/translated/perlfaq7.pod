...
Puede usar la función tainted() del módulo Scalar::Util, disponible desde CPAN (o incluido con Perl desde la distribución de 5.8.0).
...
Si quiere sobrecargar un operador Perl, tal como C<+> o C<**>, entonces querrá usar el I<pragma> C<use overload>, documentado en L<overload>.
...
Llamar a la subrutina con paréntesis, con o sin argumentos, no usan el C<@_> actual. Cambiar el ejemplo para poner paréntesis después de la llamada a C<foo>, cambia el programa:
...
Aun así, la mayor parte del tiempo, no debería preocuparse de qué paquete bendijo a un objeto, mientras que él diga que hereda de esa clase:
...
O, si está usando una distribución reciente de Perl, puede usar en su lugar la función Symbol::delete_package().
...
Así que, en algunas ocasiones, podría querer usar referencias simbólicas para manipular la tabla de símbolos directamente. Esto no importa para formatos, gestores y subrutinas, porque son siempre globales: no puede usar my() con ellos.
...
El mensaje de "I<bad interpreter>" viene del I<shell>, no de perl. El mensaje actual puede variar dependiendo de su plataforma, I<shell>, y configuraciones regionales.
