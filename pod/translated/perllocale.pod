...
Sin embargo, es posible computar los datos de la configuración regional POSIX desde ellos, y versiones anteriores de CLDR ya se lo han extraído para usted como configuraciones regionales UTF-8 en L<http://unicode.org/Public/cldr/2.0.1/>).
...
Si quiere que una aplicación Perl procese y presente sus datos de acuerdo con una configuración regional en particular, el código de la aplicación debe incluir el I<pragma> S<C<use locale>> (vea L<El I<pragma> "use locale">) en el lugar apropiado, y B<al menos uno> de los siguientes debe ser cierto:
...
=head1 USAR CONFIGURACIONES REGIONALES

=head2 El I<pragma> C<"use locale">

¡ATENCIÓN!  NO use este I<pragma> en I<scripts> que tengan activadas múltiples L<threads|threads>.  La configuración regional no es local a un único hilo.
Otro hilo puede cambiar la configuración regional en cualquier momento, que podría causar como mínimo que un determinado hilo esté operando en una configuración regional inesperada.  En algunas plataformas, puede ocurrir fallos de segmentación.  El cambio de configuración regional no necesita ser explícito; algunas operaciones hacen que perl cambie la configuración regional por sí mismo.  Queda expuesto teniendo simplemente un C<"use locale">.

Por defecto, el propio Perl (excepto el módulo L<POSIX>) ignora la configuración regional en vigor.  El I<pragma> S<C<use locale>> le pide a Perl que utilice la configuración regional actual para algunas operaciones.
A partir de v5.16, existe unos parámetros opcionales para este I<pragma>, descritos más abajo, que restringe qué operaciones quedan afectadas por él.
...
Se pueden compilar patrones de expresiones regulares utilizando L<qrE<sol>E<sol>|perlop/qrE<sol>STRINGE<sol>msixpodualn> para utilizarse más tarde en las coincidencias.  De nuevo, si la compilación se hace o no dentro del ámbito de C<use locale> determina el comportamiento de la coincidencia, no si las coincidencias se realizan dentro de tal ámbito o no.
...
El comportamiento por defecto se restaura con el I<pragma> S<C<no locale>>, o al llegar al final del bloque que encierra a C<use locale>.
...
A partir de Perl v5.16 de una forma muy limitada, y más generalmente en v5.22, puede restringir en qué categoría o categorías están activadas para esta instancia particular del I<pragma> añadiéndole parámetros.  Por ejemplo,
...
Ya que Perl, actualmente, no hace nada con la categoría C<LC_MONETARY>, especificar C<:monetary> no hace nada de forma efectiva.  Algunos sistemas tiene otras categorías, como C<LC_PAPER_SIZE>, pero Perl tampoco sabe nada sobre ellas, y no hay forma de especificarlas en los argumentos de este I<pragma>.
...
Antes de v5.22 solo estaba disponible una forma de I<pragma> con argumentos:
...
¡ATENCIÓN!  NO use esta función en un L<hilo|threads>.  La configuración regional cambiará en todos los hilos al mismo tiempo, y su hilo debería hacer una pausa -a través de su sistema operativo-, y otra iniciarse. Ese hilo no tendrá la configuración regional que se espera.  En algunas plataformas, existe una condición de carrera que lleva a fallos de segmentación si dos hilos llaman a esta función casi simultáneamente.
...
=head2 Buscar configuraciones regionales
...
=head2 Comprobar configuraciones regionales rotas
...
=head2 Arreglar temporalmente los problemas de las configuraciones regionales
...
En primer lugar, vea más arriba, en este documento, sobre L<Buscar configuraciones regionales>.  Eso indica cómo encontrar qué configuraciones regionales están realmente soportadas -y lo más importante, instaladas- en su sistema.  En nuestro ejemplo de mensaje de error, las variables de entorno que afectan a la configuración regional están listadas en el orden de importancia decreciente (y las variables que no estén definidas, no importan).  Por lo tanto, tener LC_ALL puesto a "En_US" debe haber sido una mala elección, como lo demuestra el mensaje de error.  En primer lugar trate de arreglar la configuración regional listada en primer lugar.
...
Compare esto con los caracteres que ve y su orden si declara expresamente que la configuración regional debe ignorarse:
...
Para las distribuciones v5.16 y v5.18, C<S<use locale 'not_characters>> se podría usar como solución temporal (vea L</Unicode y UTF-8>).
...
Pero, una vez más, note que Perl al interaccionar, puede usar algunas de ellas, incluyendo extensiones que estén fuera de la distribución estándar de Perl, y por el sistema operativo y sus utilidades.  Note, especialmente, que el valor de cadena de C<$!> y los mensajes de error propuestos por utilidades externas pueden quedar modificados por C<LC_MESSAGES>.  Si quiere tener códigos de error portables, utilice C<%!>.  Vea L<Errno>.
...
Todos los subpatrones, tanto si se entregan como resultado en un contexto de lista, o como C<$1>, I<etc.>, quedan contaminados si un C<use locale> que incluya C<LC_CTYPE> está en vigor, y el subpatrón de expresión regular contiene una construcción dependiente de la configuración regional.  Estas construcciones incluyen C<\w> (para coincidir con un carácter alfanumérico), C<\W> (carácter no-alfanumérico), C<\b> y C<\B> (márgenes y no-márgenes de palabra, que dependen de lo que coinciden con C<\w> y C<\W>), C<\s> (carácter espacio en blanco), C<\S> (no carácter espacio en blanco), C<\d> y C<\D> (dígitos y no-dígitos), y las clases de caracteres POSIX, tales como C<[:alpha:]> (vea L<perlrecharclass/POSIX Character Classes>).
...
El primer programa, que ignora su configuración regional, no se ejecutará: un valor tomado directamente de la línea de comandos no puede utilizarse para nombrar un archivo de salida cuando los controles de contaminación están habilitados.
...
Se puede obligar a que el programa haga un "lavado" del valor contaminado llevándolo a través de una expresión regular: el segundo ejemplo -que todavía ignora la información de la configuración regional- se ejecuta, creando el archivo con el nombre indicado en su línea de comandos, si lo puede hacer.
...
y también cómo las cadenas se interpretan como números por C<POSIX::strtod()>:
...
(Nuevas versiones de CLDR le requieren que calcule los datos POSIX por sí mismo.
Vea L<http://unicode.org/Public/cldr/latest/>).
...
Para configuraciones regionales de un solo byte, Perl, en general, asume la postura de utilizar las reglas de la configuración regional en códigos de carácter que pueden caber en un solo byte, y las reglas Unicode para aquellos que no pueden (aunque esto no se aplica uniformemente, vea la nota al final de esta sección).  fuzzy Esto evita muchos problemas en configuraciones regionales que no son UTF-8.  Supongamos que la configuración regional es ISO8859-7, Griego.  El carácter en 0xD7 es una Chi mayúscula. Sin embargo, en la configuración regional ISO8859-1, Latin1, es un signo de multiplicación.  La expresión regular de clase de caracteres POSIX C<[[:alpha:]]>, por arte de magia coincidirá con 0xD7 en la configuración regional de Griego, pero no en la Latin1.
...
Las cosas se leen como UTF-8, que normalmente implica una interpretación Unicode, pero la presencia de una configuración regional hace que se interpreten, en su lugar, con esa configuración regional.  Por ejemplo, un código de carácter 0xD7 en la entrada Unicode, lo que debería significar el signo de multiplicación, no se interpretará por Perl de esa manera en la configuración regional griega.  Esto no es un problema si tiene la seguridad que todas las configuraciones regionales siempre sean y sólo sean ISO8859-1, o, si no tiene una deficiente biblioteca C, una configuración regional UTF-8.
...
Para consideraciones especiales cuando Perl se integra en programas C, vea L<perlembed/Usando Perl integrado con configuraciones regionales POSIX>.
