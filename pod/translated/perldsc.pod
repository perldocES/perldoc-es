...
Perl nos permite tener estructuras de datos complejos.  Ahora puede escribir algo como lo siguiente, y de repente, ¡tiene una matriz de tres dimensiones!
...
Esto se debe a que Perl no (nunca), implícitamente, desreferencia sus variables.
...
Aquí está el caso de tomar una referencia a la misma ubicación de memoria, una y otra vez:
    ...
    # Sin usar strict o teniendo fuera del ámbito, una my @array;
    # declaración.
...
Los corchetes hacen referencia a un nuevo array con una I<copia> de lo que está en @array en el momento de la asignación.  Esto es lo que quiere.
...
En resumen:
    ...
    $AoA[$i] = [ @array ];     # generalmente, lo mejor
...
Puede utilizar el comando C<x> del depurador para volcar estructuras de datos complejos.
Por ejemplo, dada la asignación anterior a $AoA, aquí está la salida del depurador:
...
=head2 Acceso e impresión de un HASH DE HASHES
 ...
 # imprime todo ordenado por el número de miembros
 foreach $familia ( sort { keys %{$HoH{$b}} <=> keys %{$HoH{$a}} }
                                                             keys %HoH )
 {
     print "$familia: { ";
     for $rol ( sort keys %{ $HoH{$familia} } ) {
...
Aquí hay un ejemplo que muestra cómo crear y usar un registro cuyos campos pueden ordenarse de diferentes formas:
     ...
     %TV = (
        picapiedra  => {
            ...
            miembros => [
                ...
                { nombre => "pebbles", rol => "niño",   edad  =>  4, },
            ],
        },
...
=head2 Generación de un HASH DE REGISTROS COMPLEJOS
     ...
     foreach $familia ( keys %TV ) {
         print "los $familia";
         print " se emiten @{ $TV{$familia}{noches} }\n";
         print "sus miembros son:\n";
         for $quien ( @{ $TV{$familia}{miembros} } ) {
             print " $quien->{nombre} ($quien->{rol}), edad $quien->{edad}\n";
         }
         print "resulta que $TV{$familia}{lider} tiene ";
         print scalar ( @{ $TV{$familia}{hijos} } ), " hijos, que se llaman ";
