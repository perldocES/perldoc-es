...
C<m/patrón/msixpogcdualn> busca en una cadena una coincidencia con un patrón, aplicando las opciones indicadas.
...
Las clases de carácter POSIX y sus equivalentes Perl:
 ...
 -----------------------------------------------------------------------
 alnum   PosixAlnum       XPosixAlnum            'alpha' más 'digit'
 ...
 graph   PosixGraph       XPosixGraph            'alnum' más 'punct'
 lower   PosixLower       XPosixLower            Caracteres en minúscula
 print   PosixPrint       XPosixPrint            'graph' más 'space',
                                                   pero no cualquier Control
 punct   PosixPunct       XPosixPunct            Puntuación y símbolos
                                                   en el rango del ASCII; con la
                                                   puntuación excluida
 space   PosixSpace       XPosixSpace   \s       Espacio en blanco
 upper   PosixUpper       XPosixUpper            Caracteres en mayúscula
 word    PosixWord        XPosixWord    \w       'alnum' más marcas Unicode
                                                    más conectores, como
                                                    '_' (extensión de Perl)
...
=head2 CONSTRUCCIONES EXTENDIDAS
   ...
   (?{ código })     Código integrado, el valor devuelto se almacena en $^R
   ...
   (?(cond)sí)       Expresión condicional, donde "cond" puede ser:
                     ...
                     (?!pat)   inspección negativa hacia adelante
                     (?<=pat)  inspección hacia atrás
                     (?<!pat)  inspección negativa hacia atrás
...
Las primeras de estas cinco son como las secuencias de escape C<\L>, C<\l>, C<\U>, C<\u> y C<\F>.  Para titulación, vea L</Tamaño de caja titular>; Para plegamiento, vea L</Plegamiento>.

=head2 TERMINOLOGÍA

=head3 Tamaño de caja titular
