...
Este documento describe las diferencias entre las distribuciones 5.8.0 y 5.8.1.

Si está actualizando desde una distribución anterior como la 5.6.1, primero lea L<perl58delta>, que describe las diferencias entre 5.6.0 y la 5.8.0.

En caso de que usted se está preguntando sobre 5.6.1, fue una corrección de errores, idéntica a la distribución de desarrollo 5.7.1.  ¿Confundido?  Esta línea de tiempo, con suerte, le ayudará un poco: lista las distribuciones importantes, sus distribuciones de mantenimiento y las distribuciones de desarrollo.
...
La opción -C ha cambiado de una forma incompatible.  La semántica anterior de esta opción sólo tiene sentido en Win32 y sólo con "use utf8" en el universo de distribuciones 5.6.x, y no tienen sentido para la implementación de Unicode en 5.8.0.  Dado que esta opción quizás no podría haberse utilizado más por cualquier persona, se ha reutilizado.  El comportamiento que esta opción se activaba en las distribuciones 5.6.x se puede soportar de manera transparente, dependiendo de la moda, en una próxima distribución.
...
El problema es que la gran mayoría de la gente no lo era: por ejemplo, en las distribuciones RedHat 8 y 9, la configuración local B<por defecto> es UTF-8, por lo que todos los usuarios de RedHat obtenían identificadores de archivo UTF-8, lo quisieran o no.
El dolor se intensificó por que la implementación de Unicode de Perl 5.8.0 (todavía) tenía errores desagradables, especialmente en relación con el uso de s// y tr///.  (Errores que se han corregido en 5.8.1)
...
Por lo tanto, perl 5.8.1 introduce una "puerta trasera" para restaurar el comportamiento pre-5.8.0 (pre-5.7.3, en realidad) de las señales.  Sólo tiene que configurar la variable de entorno PERL_SIGNALS a C<unsafe>, e inmediatamente el antiguo (y peligroso) comportamiento de la gestión de señales volverá.  Vea L<perlrun/PERL_SIGNALS> y L<perlipc/"Señales aplazadas (Señales seguras)">.
...
La copia de la base de datos de caracteres Unicode incluidos en Perl 5.8 se ha actualizada desde la 3.2.0 a la 4.0.0.  Esto significa que, por ejemplo, las propiedades de caracteres Unicode son como en Unicode 4.0.0.
...
Si un I<socket> se cierra por el servidor mientras se está imprimiendo en él, el cliente recibe ahora una SIGPIPE.  Aunque esta nueva característica no fue planeada, apareció de forma natural de los cambios que se hicieron a PerlIO, y se considera una característica accidental.
...
Los siguientes módulos y pragmas se han actualizado desde Perl 5.8.0:
...
Actualizaciones significativas en la funcionalidad del I<pragma> de codificación (formatos tr/// y del identificador de archivo de datos DATA).

Si un identificador de archivo se ha marcado para tener una cierta codificación, los caracteres no traducibles se detectan en la entrada, no después (cuando los datos dañados se estén utilizando).

La tabla de conversión ISO 8859-6 se ha corregido (el 0x30..0x39 erróneamente estaba asignado a U+0660..U+0669, en lugar de U+0030..U+0039).  La conversión GSM 03.38 no gestiona correctamente las secuencias de escape.  La codificación UTF-7 ha sido añadida (haciendo que Encode tenga las características completas con Unicode::String).
...
Las versiones anteriores del I<pragma> strict no comprobaban de forma implícita los parámetros pasados a sus rutinas "import" (use) y "unimport" (no).
...
Ahora es mucho más exigente con la salida extra o con la que falta de los I<scripts> de test.
...
Muchas fugas de memoria se han corregido.
...
perl573delta se ha añadido a la lista de las diferencias entre las distribuciones de desarrollo (ya bastante obsoletas) 5.7.2 y 5.7.3.
...
Ahora se pueden especificar directorios de instalación para las páginas man y HTML, y I<scripts> para el propio equipo y para el instalador del sistema.  Vea F<INSTALL>.
...
versiones 3.x de gcc introdujeron una nueva advertencia que causaba mucho ruido en la compilación Perl: C<gcc -Ialreadyknowndirectory (advertencia: cambiando el órden de búsqueda)>.  Esta advertencia se ha eliminado para no confundir a Configure de esos directorios antes de la compilación.
...
En las distribuciones más recientes de FreeBSD Perl 5.8.0 la compilación falló debido a que trata de utilizar F<malloc.h>, que en FreeBSD es solo un archivo ficticio, y es un error fatal incluso el tratar de usarlo.  Ahora ya no se utiliza F<malloc.h>.

Se sabe que Perl se genera también ahora en Hitachi HI-UXMPP.

Se sabe que Perl se genera también ahora en LynxOS.

Mac OS X ahora se instala con el número de versión de Perl integrado en los nombres de directorio de la instalación para facilitar la actualización por parte del usuario que compiló Perl, y los directorios de instalación, en general, son más estándar.
...
Perl se ha adaptado al entorno de IBM OS/400 PASE.  La mejor manera de generar un Perl para PASE es utilizar un host AIX como entorno de compilación cruzado.  Vea README.os400.
...
(Los programas Perl siempre han sido capaces de tener más que eso, es sólo que los números de línea de los informes de errores y advertencias son "reiniciados").  Mientras que I<scripts> tan grandes como estos, por lo general, indican la necesidad de repensar un poco el código, I<scripts> de Perl como estos existen, por ejemplo, de resultas de un código generado.  Ahora, los números de línea puede ir hasta 4294967296, o 2**32.
...
La aritmética de punteros errónea se ha corregido.
...
La forma de cuatro argumentos de select() no preservaba $! (errno) correctamente cuando se han producido errores en la llamada subyacente.  Esto se ha arreglado.

=item *

El problema "CR CR LF" se ha arreglado, binmode(FH ":crlf") es ahora, efectivamente, una no-operación.
...
Los ejemplos integrados en L<perlembed> se han revisado para estar al día y ser consistentes: por ejemplo, el uso correcto de PERL_SYS_INIT3() y PERL_SYS_TERM().

Una remodelación extensa del código de control (el código responsable de las variables léxicas) se ha llevado a cabo por Dave Mitchell.

Una amplia labor en la v-strings por parte de John Peacock.

Longitud y posición de la caché UTF-8: para acelerar la tramitación de escalares Unicode (UTF-8), se introdujo una memoria caché.  Existen problemas potenciales si una extensión no pasa por la API oficial y modifica directamente el PV de un SV: el caché UTF-8 no se borra como debiera.
...
Ciertas API C del núcleo de Perl, como C<Perl_list> ya no están disponibles sin su prefijo C<Perl_>.  Si su módulo XS deja de funcionar debido a que algunas funciones no se pueden encontrar, en muchos casos una solución sencilla es añadir el prefijo C<Perl_> a la función y el contexto del hilo C<aTHX_> como primer argumento de la llamada a la función.  Esta también es la forma en que siempre se debería haber hecho: dejar que los formularios Perl agotasen el núcleo fue un accidente.  Para una integración más limpia también puede forzar esto para todas las API definiendo, en tiempo de compilación, el B<define> PERL_NO_SHORT_NAMES para cpp.
...
Las macros internas de depueración Perl DEBUG() y DEB() se han renombrado a PERL_DEBUG() y PERL_DEB() para evitar conflictos en el espacio de nombres.
...
IBM z/OS y otras plataformas EBCDIC siguen siendo problemáticas en relación con la compatibilidad con Unicode.  Muchas pruebas de Unicode se omiten cuando en realidad deberían arreglarse.
...
Con ciertas distribuciones del compilador de C de HP (por ejemplo, B.11.11.02) obtendrá muchas advertencias como esta (líneas ajustadas para facilitar la lectura):
...
La prueba t/uni/tr_7jis.t se sabe que da un informe de fallo bajo 'make test' o la prueba se cuelga con determinadas distribuciones de IRIX (por lo menos la IRIX 6.5 y compiladores MIPSpro versión 7.3.1.1m), pero si se ejecuta manualmente la prueba pasa de forma completa.
...
En las últimas distribuciones de Tru64 (por ejemplo, v5.1B o posterior) gcc no se puede utilizar para compilar un Perl con soporte de hilos de ejecución (-Duseithreads) debido a que el archivo del sistema C<< <pthread.h> >> no sabe nada sobre gcc.

=head2 Win32: sysopen, sysread, syswrite

A partir de la distribución 5.8.0, sysopen()/sysread()/syswrite() no se comportan como lo hacían antes en 5.6.1 y anteriores, con respecto al modo "texto".
Estos funciones predefinidas, ahora, funcionan siempre en modo "binario" (incluso si se pasó a sysopen() el indicador O_TEXT, o si se utilizó binmode() con el identificador de archivo).  Tenga en cuenta que este problema sólo afecta a los archivos de disco, ya que los I<sockets> y tuberías siempre han estado en modo "binario" en la versión para Windows.  Ya que este comportamiento se considera actualmente un error, un comportamiento compatible puede ser re-introducido en una futura distribución.  Hasta entonces, el uso de sysopen(), sysread() y syswrite() no está soportado para las operaciones en modo "texto".

=head1 Direcciones futuras

Las siguientes cosas B<pueden> suceder en el futuro.  Las primeras distribuciones a disposición del público de estas características serán publicadas en la distribución desarrollador Perl 5.9.x, culminando con la distribución Perl 5.10.0.  Estas son nuestras mejores estimaciones por el momento: nos reservamos el derecho de reconsiderarlo.

=over 4

=item *

PerlIO se convertirá en el predeterminado.  En la actualidad (en Perl 5.8.x) la biblioteca stdio todavía se utiliza si Perl piensa que puede usar ciertos trucos para hacer que stdio vaya B<realmente> más rápido.  Para futuras distribuciones nuestro objetivo es hacer que PerlIO vaya aún más rápido.
