...
Antes de Perl 5.8.0 era necesario hacer lo menos posible en el gestor; note que todo lo que hacemos es establecer una variable global y luego levanta una excepción.  Es porque en la mayor parte de los sistemas, las bibliotecas no son reentrantes; en particular, la reserva de memoria y las rutinas de E/S no lo son.  Eso significa que hacer I<cualquier cosa> en su gestor podría, en teoría, disparar un fallo de memoria y un subsecuente volcado del núcleo. Vea L</señales diferidas (Señales seguras)> más abajo.
...
=head2 Señales diferidas (Señales seguras)
...
Entonces en los puntos estratégicos "seguros" en el intérprete Perl (p.e. cuando está a punto de ejecutar un nuevo código de operación) se comprueban los indicadores y se ejecuta el gestor de %SIG a nivel de Perl. El esquema "diferido" permite mucha mayor flexibilidad en el código de los gestores de señal cuando sabemos que el intérprete Perl está en un estado seguro, y que no estamos en una función de biblioteca del sistema cuando se llama al gestor.  Sin embargo la implementación difiere de los Perl anteriores de las formas siguientes:
...
Podría notar que podría usar las comillas inversas para conseguir el mismo efecto de abrir una tubería en modo lectura:
...
Otro uso común para esta construcción es cuando necesita ejecutar algo sin la interferencia del shell.  Con system(), es inmediato, pero no puede usar una tubería o comillas inversas de forma segura.
...
En el ejemplo anterior, el verdadero padre no quiere escribir al identificador de archivo ESCRITOR, y por eso lo cierra.  Aún así, ya que ESCRITOR se abrió con C<open FH, "|-">, tiene un comportamiento especial: cerrarlo llama a waitpid() (vea L<perlfunc/waitpid>), que espera a que el subproceso termine.  Si el proceso hijo termina esperando por que suceda algo en la sección marcada como "ahora hacer otra cosa", tiene un bloqueo.
...
Esto es lo preferido cuando desea evitar que el shell interprete metacaracteres que podrían estar en su línea de comando.
...
Si mira en su código, verá que open2() usa primitivas de llamadas al sistema, de bajo nivel como pipe() y exec() para crear todas las conexiones.  A pesar de que podría ser más eficiente usar socketpair(), podría ser incluso menos portátil de lo que ya es. Las funciones open2() y open3() no es probable que funcionen en todos los sistemas excepto en un sistema Unix, o al menos en uno que sea conforme POSIX.
...
Dentro del bucle while llamamos a accept() y comprobamos si devuelve un valor falso.  Eso, normalmente, indicaría que se necesita reportar un error del sistema.  Sin embargo, la introducción de señales seguras (vea L</Señales diferidas (Señales seguras)> más arriba) en Perl 5.8.0 significa que accept() podría también interrumpirse cuando el proceso recibe una señal.
