...
Establece una acción (comando Perl) para que se ejecute después de que salga el símbolo del sistema del depurador que sale justo después de regresar de ejecutar el script.  Se puede entrar un comando de varias líneas escapando los caracteres de nueva línea (apostamos  que no la habría adivinado).
