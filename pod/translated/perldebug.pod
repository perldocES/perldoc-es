...
La secuencia de pasos tomados por el depurador es
  ...
  4. presenta un símbolo del sistema al usuario si es un punto de parada o un paso simple
  5. evalúa la línea
...
Lista todas los comandos Perl de acciones, anteriores a la presentación del símbolo del sistema.

=item < [ comando ]
X<< debugger command, < >> X<comando del depurador, < >>

Establece una acción (comando Perl) para que se ejecute antes de todas las apariciones del símbolo del sistema del depurador.
Se puede entrar un comando de varias líneas escapando los caracteres de nueva línea.

=item < *
X<< debugger command, < >> X<< comando del depurador, < >>

Borra todas los comandos Perl de las acciones anteriores a la presentación del símbolo del sistema.

=item << comando
X<< debugger command, << >> X<< comando del depurador, << >>

Añade una acción (comando Perl) para que se ejecute antes de todas las apariciones del símbolo del sistema del depurador.
Se puede entrar un comando de varias líneas escapando los caracteres de nueva línea.

=item > ?
X<< debugger command, > >> X<< comando del depurador, > >>

Lista todas los comandos Perl de acciones, posteriores a la presentación del símbolo del sistema.

=item > comando
X<< debugger command, > >> X<< comando del depurador, > >>

Establece una acción (comando Perl) para que se ejecute después de que salga el símbolo del sistema del depurador que sale justo después de regresar de ejecutar el I<script>.  Se puede entrar un comando de varias líneas escapando los caracteres de nueva línea (apostamos  que no la habría adivinado).

=item > *
X<< debugger command, > >> X<< comando del depurador, > >>

Borra todas los comandos Perl de las acciones posteriores a la presentación del símbolo del sistema.

=item >> comando
X<<< debugger command, >> >>> X<<< comando del depurador, >> >>>

Añade una acción (comando Perl) para que se ejecute después de que salga el símbolo del sistema del depurador que sale justo después de regresar de ejecutar el I<script>.  Se puede entrar un comando de varias líneas escapando los caracteres de nueva línea.

=item { ?
X<debugger command, {> X<comando del depurador, {>

Lista los comandos del depurador antes del símbolo del sistema.

=item { [ comando ]

Establece una acción (comando del depurador) para que se ejecute antes de todas las apariciones del símbolo del sistema del depurador.
...
Borra todos los comandos del depurador antes del símbolo del sistema.

=item {{ comando
X<debugger command, {{> X<comando del depurador, {{>

Añade una acción (comando del depurador) para que se ejecute antes de todas las apariciones del símbolo del sistema del depurador.
...
Ejecuta cmd en un subproceso (lee desde DB::IN, escriba hacia DB::OUT). Vea también C<o shellBang>.  Note que se utilizará el I<shell> actual del usuario (bueno, su variable C<$ENV{SHELL}>), que puede interferir con una interpretación correcta del estado de salida o de la información de la señal o volcado del núcleo.
...
Los caracteres utilizados para recordar un comando o lanzar una I<shell>.  Por defecto, los dos se establecen a C<!>, que es algo desafortunado.
...
Ejecuta Tk mientras muestra el símbolo del sistema (con ReadLine).
...
Después de que se lea el archivo rc, el depurador lee la variable de entorno C<$ENV{PERLDB_OPTS}> y lo interpreta como si se hubiera entrado el resto de una línea "O ..." en el símbolo del sistema del depurador.  Puede colocar allí las opciones de inicialización C<TTY>, C<noTTY>, C<ReadLine> y C<NonStop>.
...
Otros ejemplos incluyen (usando la sintaxis de I<shell> estándar para mostrar los valores de las variables de entorno):

  $ ( PERLDB_OPTS="NonStop frame=1 AutoTrace LineInfo=tperl.out"
      perl -d miprograma )

que puede ser útil para la depuración de un programa que use por sí mismo C<Term::ReadLine>.  No olvide desenganchar su I<shell> de la TTY en la ventana que corresponda a F</dev/ttyXX>, digamos, por medio de un comando como
...
=item Prompt (símbolo del sistema)

El símbolo del sistema del depurador es algo así
...
El cual intentará establecer puntos de parada en las líneas 4 y 6 inmediatamente después de la inicialización del depurador. Note que @DB::typeahead no es un interfaz soportado, y está sujeto a cambios en futuras distribuciones.
...
Si se encuentra en una llamada del sistema lenta (como C<wait>, C<accept>, o C<read> desde su teclado o un I<socket>) y no ha establecido su propio gestor C<$SIG{INT}>, entonces no será capaz de hacer un CTRL-C para regresar al depurador, porque el propio gestor C<$SIG{INT}> del depurador no entiende que necesita levantar una excepción a longjmp(3) fuera de las lentas llamadas del sistema.
