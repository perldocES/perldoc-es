...
Con los métodos 2 y 3, Perl inicia el análisis del archivo de entrada desde el comienzo, a menos que haya especificado una opción B<-x>, en cuyo caso escanea por la primera línea que comience por C<#!> y contenga la palabra "perl", y arrancará desde allí.  Esto es útil para ejecutar un programa integrado en un correo grande.  (En este caso se indicará el final del programa, utilizando el I<token> C<__END__>).

La línea C<#!> es siempre examinada para buscar las opciones mientras la línea está siendo analizada.  Por lo tanto, si está en una máquina que sólo permite un argumento con la línea C<#!>, o peor aún, ni siquiera reconoce la línea C<#!>, todavía puede obtener un comportamiento consistente de las opciones independientemente de cómo Perl se haya invocado, incluso si B<-x> fue utilizado para encontrar el inicio del programa.

Debido a que, históricamente, algunos sistemas operativos cortan, silenciosamente, la interpretación de la línea C<#!> después de 32 caracteres, algunas opciones pueden pasarse a la línea de comandos, y algunas otras, no; incluso se podría obtener un "-" sin su carácter, si no tiene cuidado.
...
Si la línea C<#!> no contiene la palabra "perl" ni la palabra "indir", el programa que se nombra después de C<#!> se ejecuta en lugar del intérprete de Perl.  Esto es algo extraño, pero ayuda a personas en máquinas que no pueden trabajar con C<#!>, porque ellas pueden indicar a un programa que su SHELL es F</usr/bin/perl>, y Perl, entonces, despachará, por ellos, el programa al intérprete correcto.

Después de localizar a su programa, Perl compila el programa entero a una forma interna.  Si existe algún error de compilación, ni se intenta la ejecución del programa.  (Esto es muy distinto del típico I<script> I<shell>, que se ejecutará sin pensarlo hasta encontrar el primer error de sintaxis).

Si el programa es sintácticamente correcto, se ejecuta.  Si el programa se ejecuta hasta el final sin llegar a un operador exit() o die(), se ejecuta un implícito C<exit(0)> para indicar un completado exitoso.

=head2 #! y entrecomillado en sistemas distintos del Unix
X<hashbang> X<#!>

La técnica C<#!> del Unix puede simularse en otros sistemas:
...
El problema es que nada de esto es fiable: depende del comando e es incluso posible que nada funcione.  Si I<4DOS> fuera el I<shell> de comandos, esto serviría mejor:
...
Como con todos los comandos estándares, una opción de un sólo carácter puede agruparse con la siguiente opción, si la hubiera.
...
También puede especificar el carácter separador usando notación hexadecimal: B<-0xI<HHH...>>, donde C<I<H>> son dígitos hexadecimales válidos.  A diferencia de con la forma octal, ésta puede usarse para especificar cualquier carácter Unicode, incluso lo que están más allá de 0xFF.  Así que si I<verdaderamente> quiere usar un separador de registro 0777, especifíquelo como B<-0x1FF>.  (Esto significa que no puede usar la opción B<-x> con un nombre de directorio que consista en dígitos hexadecimales, o sino, Perl pensará que ha especificado un número hex relativo a B<-0>).
...
A partir de 5.8.1, el B<-C> puede ser seguido tanto por un número como una lista de letras de opciones.  Estas letras, sus valores numéricos, y efectos, son como sigue; listar las letras es lo mismo que sumar los números.
    ...
    I     1   STDIN se asume que está en UTF-8
...
Las opciones C<io> quieren decir que cualquier subsecuente open() (o similares operaciones de E/S) en el actual ámbito del archivo tendrá aplicado una implícita capa PerlIO C<:utf8>, en otras palabras, se espera UTF-8 en cualquier flujo de entrada, y se genera UTF-8 en cualquier flujo de salida.  Esto queda marcado como por defecto, mientras que las capas explícitas en open() y con binmode() se pueden manipular los flujos de la forma usual.
...
B<Nota:> Desde perl 5.10.1, si la opción B<-C> es usada en la línea C<#!>, también debe especificarse en la línea de comandos, debido a que los flujos estándares se inicializan en ese punto, en la ejecución del intérprete perl.
También puede usar binmode() para establecer la codificación de un flujo de E/S.

=item B<-c>
X<-c>

hace que Perl comprueba la sintaxis del programa y salga sin ejecutarlo.  Realmente, I<ejecutará> los bloques C<BEGIN>, C<UNITCHECK>, o C<CHECK> y cualquier instrucción C<use>: se les considera que ocurren fuera de la ejecución de su programa.  Bloques C<INIT> y C<END>, sin embargo, se obvian.
...
activa opciones de depuración.  Para mirar cómo se ejecuta su programa, use B<-Dtls>.  (Esto funciona solo si el soporte para depuración fue compilado en su Perl).  Otro valor interesante es B<-Dx>, que lista el árbol de sintaxis compilado.  Y B<-Dr> muestra las expresiones regulares compiladas; el formato de salida se explica en L<perldebguts>.
...
Si solo está intentando imprimir cada línea del código Perl que está ejecutando, de la misma forma que C<sh -x> se ofrece para los I<script> del I<shell>, no puede usar la opción B<-D> con Perl.  En lugar de eso
...
puede usarse para entrar un programa de una línea.  Si se indica B<-e>, Perl no mirará por un nombre de archivo en la lista de argumentos.  Se pueden indicar múltiples comandos B<-e> para generar un I<script> multilínea.  Asegúrese de usar caracteres de punto y coma tal y como haría con un programa normal.
...
Puede, por ejemplo, usarse para añadir entradas al array @INC para hacer que Perl busque módulos en lugares no estándares.
...
Debido a que es un C<do> real (no un C<require>), F<sitecustomize.pl> no retorna un valor verdadero. El código se ejecuta en el paquete C<main>, en su propio ámbito léxico. Sin embargo, si el I<script> muere, C<$@> no se actualizará.

El valor de C<$Config{sitelib}> también está determinado en el código C, no leído desde C<Config.pm>, que no se carga.

El código se ejecuta I<muy> prontamente. Por ejemplo, cualquier cambio hecho a C<@INC> se mostrará en la salida de `perl -V`. Naturalmente, los bloques C<END> serán igualmente ejecutados muy tardíamente.

Para determinar en tiempo de ejecución si esta capacidad se ha compilado en su perl, puede comprobar el valor de C<$Config{usesitecustomize}>.
...
Si la extensión no contiene un C<*>, entonces es añadida al final del nombre de archivo actual como sufijo.  Si la extensión contiene uno o más caracteres C<*>, entonces cada C<*> se reemplaza por el nombre del archivo actual.  En términos Perl, se podría pensar que se está haciendo esto:
...
excepto que la opción B<-i> no necesita comparar $ARGV a $oldargv para saber cuándo el nombre de archivo ha cambiado.  Aun así, usa ARGVOUT para el identificador de archivo seleccionado.  Tenga en cuenta que STDOUT se restaura como el identificador de archivo por defecto, después del bucle.
...
Tenga en cuenta que debido a que B<-i> renombra o elimina el archivo original antes de crear un archivo nuevo con el mismo nombre, los enlaces tipo Unix, blandos y duros, no se preservará.

Finalmente, la opción B<-i> no impide la ejecución cuándo no se indica ningún archivo en la línea de comandos.  En este caso, no se realiza ninguna copia de seguridad (el archivo original no puede, naturalmente, determinarse) y el procesamiento procede de STDIN a STDOUT, que es lo esperado.
...
habilita el procesamiento automático de fin de línea.  Tiene dos efectos separados.  Primero, automáticamente chomps C<$/> (el separador de registros de entrada) cuándo se utiliza con B<-n> o B<-p>.  Segundo, asigna C<$\> (el separador de registros de salida) al valor I<núm_oct>, de modo que cualesquiera instrucciones de impresión tendrán este separador añadido.
...
Si el primer carácter después de B<-M> o B<-m> es un guion (B<->) entonces el 'use' se reemplaza por un 'no'.
...
Esto es más rápido que utilizar la opción B<-exec> de I<find> porque no tiene que arrancar un proceso por cada nombre de archivo encontrado (pero no es más rápido que usar la opción B<-delete>, disponible en las nuevas versiones de I<find>).
Adolece del error de perder los caracteres de nueva línea en los nombres de las rutas de archivos, que puede arreglar si sigue el ejemplo que hay en B<-0>.

Los bloques C<BEGIN> y C<END> pueden usarse para capturar el control antes de o después del
...
Si un archivo nombrado por un argumento no puede ser abierto por alguna razón, Perl se lo advierte y pasa al archivo siguiente.  Note que las líneas son impresas automáticamente.  Un error que ocurra durante la impresión se trata como fatal.  Para suprimir la impresión use la opción B<-n>.  Una opción B<-p> sobrescribe una B<-n>.

Los bloques C<BEGIN> y C<END> pueden usarse para capturar el control antes de o después del bucle implícito, como se hace en I<awk>.

=item B<-s>
X<-s>

habilita un rudimentario análisis de opciones en la línea de comandos después del nombre de programa pero antes de cualquier argumento de nombre de archivo (o antes de un argumento B<-->).  Cualquier opción encontrada allí es eliminada de @ARGV y se pone en la correspondiente variable del programa Perl.  El programa siguiente imprime "1" si el programa se invoca con una opción B<-xyz>, y "abc" si se invoca con B<-xyz=abc>.
...
Típicamente esto es lo que se usa para emular el arranque C<#!> en plataformas que no soportan C<#!>.  Es también conveniente que cuando se depura un guión que usa C<#!>, y es normalmente encontrado por los mecanismos de búsqueda del I<shell> en $PATH.

Este ejemplo funciona en muchas plataformas que tienen un I<shell> compatible con I<shell> Bourne:

    #!/usr/bin/perl
    eval 'exec /usr/bin/perl -wS $0 ${1+"$@"}'
	    if $ejecutando_bajo_algun_shell;

El sistema ignora la primera línea y alimenta el programa a F</bin/sh>, el cual procede a probar a ejecutar el programa Perl como un guión de I<shell>.
El I<shell> ejecuta la segunda línea como un comando I<shell> normal, y así arranca el intérprete Perl.  En algunos sistemas, $0 no siempre contiene la ruta completa, por lo que B<-S> le dice a Perl que busque el programa, si es necesario.  Después de localizar el programa, Perl analiza las líneas y las ignora porque la variable $ejecutando_bajo_algun_shell no es verdadera.  Si el programa se interpretará por csh, tendrá que reemplazar C<${1+"$@"}> por C<$*>, incluso aunque esto no interprete los espacios integrados (y cosas así) en la lista de argumentos.  Para poner en marcha I<sh> en lugar de I<csh>, algunos sistemas pueden tener que reemplazar la línea C<#!> con una línea que contenga sólo un carácter de dos puntos, que será amablemente ignorado por Perl.  Otros sistemas no controlan nada de esto, y necesitan de una enrevesada construcción que funcione tanto en I<csh>, como I<sh> o Perl, tales como la siguiente:

	eval '(exit $?0)' && eval 'exec perl -wS $0 ${1+"$@"}'
	& eval 'exec /usr/bin/perl -wS $0 $argv:q'
		if $ejecutando_bajo_algun_shell;

Si el nombre del archivo suministrado contiene separadores de directorios (y por lo tanto es una ruta absoluta o relativa), y si ese archivo no se encuentra, las plataformas que añaden extensiones de archivo lo harán, y tratarán de buscar el archivo con estas extensiones añadidas, una a una.

En plataformas DOS, si el programa no contiene separadores de directorios, en primer lugar se buscará en el directorio actual antes de buscarse en el PATH.  En plataformas Unix, el programa se buscará estrictamente en PATH.
...
B<Nota: ¡Esto no es un sustituto de C<-T>!> Su destino es utilizarse I<sólo> como una ayuda temporal para el desarrollo, mientras asegura código heredado: para el código en producción real y para nuevo código seguro escrito desde cero, utilice siempre el verdadero B<-T>.
...
Además, los caracteres de dos puntos adicionales se pueden utilizar para controlar el formato.  Un carácter de dos puntos final suprime el salto de línea y el terminador ";", permitiendo integrar consultas en comandos I<shell>.  (nemotécnico: separador ":" en PATH).
...
Caracteres dos puntos precedentes y finales pueden usarse conjuntamente si necesita valores de parámetros posicionales sin los nombres.  Note que, en el caso siguiente, los parámetros C<PERL_API> se devuelven en orden alfabético.
...
imprime advertencias sobre construcciones dudosas, como nombres de variables mencionadas sólo una vez y variables escalares utilizadas antes de declararse; subrutinas redefinidas; referencias a identificadores de archivo indefinidos; identificadores de archivo abiertos en sólo lectura mientras que está intentando escribir en ellos; valores utilizados como un número que no I<parecen> números; utilizando un array como si se tratara de un escalar; si sus subrutinas tienen más de 100 niveles de profundidad de recursión; y un sinnúmero de otras cosas.

Esta opción, en realidad, sólo activa la variable global C<$^W>; normalmente, es preferible usar el I<pragma> de ámbito léxico C<use warnings>. Puede desactivar o promover en errores fatales, advertencias específicas utilizando enganches C<__WARN__>, como se describe en L<perlvar> y L<perlfunc/warn>.
...
Si se especifica un nombre de directorio, Perl cambiará a ese directorio antes de ejecutar el programa.  La opción B<-x> controla sólo la eliminación de la basura inicial.  El programa debe terminar con C<__END__> si detrás hay basura que debe ignorarse; el programa puede procesar cualquier o toda la basura a través del identificador de archivo C<DATA>, si así se desea.
...
La lista se convierte en el valor predeterminado para I<toda> E/S de Perl. En consecuencia, sólo capas predefinidas en el código pueden aparecer en esta lista, ya que las capas externas (tales como C<encoding()>) ¡necesitan de E/S para poder leerse!  Vea L<"open I<pragma>"|open> para saber cómo añadir codificaciones externas por defecto.
...
Una pseudocapa que manipula las otras capas.  Aplicar la capa C<:raw> es equivalente a llamar a C<binmode($fh)>.  Hace pasar el flujo de cada byte tal cual, sin traducción.  En particular, tanto la traducción de CRLF y la intuición de C<:utf8> desde la configuración regional, se desactivan.
...
Una pseudocapa que activa un indicador en la capa inferior para avisar a Perl que la salida debe ser en utf8 y que la entrada debe considerarse como que ya está en una forma válida de utf8. B<AVISO: no comprueba la validez y debe gestionarse con extrema cautela para la entrada, debido a que pueden ocurrir violaciones de seguridad, con codificaciones UTF-8 no acortadas, etc.> Por lo general, C<:encoding(utf8)> es la mejor opción al leer datos codificados en UTF-8.

=item :win32
X<:win32>

En plataformas Win32 esta capa I<experimental> usa "identificadores" de E/S nativos en lugar de una capa de descriptor de archivo numérico tipo Unix. Se sabe que tiene errores en esta distribución (5.14).
...
En Win32 el valor por defecto en esta distribución (5.14) es "unix crlf". El "stdio" de Win32 tiene una serie de errores/fallas en PerlIO que son un poco dependientes de la versión y del proveedor del compilador de C. Usando nuestra propia capa C<crlf> como búfer, evita estos problemas y hace las cosas más uniformes.  La capa C<crlf> proporciona la conversión CRLF, así como almacenamiento en búfer.

Esta distribución (5.14) utiliza C<unix> como la capa inferior de Win32, por lo que todavía utiliza las rutinas de descriptor numérico de archivo del compilador de C. Existe una capa experimental nativa C<win32>, que se espera que sea mejor y, finalmente, deba ser el valor por defecto en Win32.
...
Esta funcionalidad está deshabilitada para I<scripts> setuid y para I<scripts> que se ejecuten con B<-T>.
...
La variable de entorno PERL5DB sólo se usa cuando Perl se inicia con una simple opción B<-d>.
...
En las adaptaciones Win32, se puede establecer un I<shell> alternativo que Perl debe utilizar internamente para la ejecución de comandos en "comillas invertidas" o system().  Por defecto es C<cmd.exe /x/d/c> en Windows NT y C<command.com /c> en Windows 95.  El valor se considera que está separado por espacios.  Preceda cualquier carácter que necesite ser protegido, como un espacio o una barra diagonal inversa, con otra barra diagonal inversa.

Tenga en cuenta que Perl no utiliza COMSPEC para este propósito porque COMSPEC tiene un alto grado de variabilidad entre los usuarios, lo que se traduce en problemas de portabilidad.  Además, Perl puede utilizar un I<shell> que no pueda ser apto para el uso interactivo, y el establecimiento de COMSPEC a un I<shell> puede interferir con el buen funcionamiento de otros programas (que sí suelen mirar en COMSPEC para encontrar un I<shell> para el uso interactivo).
...
Sólo es relevante si Perl se compila con el C<malloc> incluido con la distribución de Perl; es decir, si C<perl -V:d_mymalloc> está "I<define>" (definido).
...
Se establece en C<"1"> para hacer que Perl resuelva I<todos> los símbolos indefinidos cuando se cargue una biblioteca dinámica.  El comportamiento por defecto es resolver los símbolos cuando se utilizan.  Establecer esta variable es útil durante las pruebas de las extensiones, ya que asegura que obtendrá un error con nombres de función mal escritos, incluso si el conjunto de pruebas no los llama.

=item PERL_ENCODING
X<PERL_ENCODING>

Si se utiliza el I<pragma> C<use encoding> sin un nombre de codificación explícito, la variable de entorno PERL_ENCODING es consultada por un nombre de codificación.
...
B<POR FAVOR, TENGA EN CUENTA: La semilla hash es información sensible>. Los hash se asignan al azar para protegerse contra los ataques locales y remotos contra el código Perl. Poniendo manualmente una semilla, esta protección puede quedar parcial o completamente perdida.
...
Disponible en Perls 5.8.1 y versiones posteriores.  Si se establece a C<"unsafe">, se restaura el comportamiento de señales como en pre-Perl-5.8.0 (que es rápido, pero inseguro).  Si se establece a C<safe>, entonces se usan señales seguras (pero aplazadas).  Vea L<perlipc/"Señales aplazadas (Señales Seguras)">.

=item PERL_UNICODE
X<PERL_UNICODE>

Equivalente a la opción de línea de comandos B<-C>.  Tenga en cuenta que esto no es una variable booleana. Definiendo este valor a C<"1">, no es la manera correcta de "activar Unicode" (sea lo que signifique).  Al contrario, puede utilizar C<"0"> para "desactivar Unicode" (o, alternativamente, limpiar PERL_UNICODE en su I<shell> antes de iniciar Perl).  Vea la descripción de la opción B<-C> para más información sobre esto.
