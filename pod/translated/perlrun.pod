...
B<Nota:> Desde perl 5.10.1, si la opción B<-C> es usada en la línea C<#!>, también debe especificarse en la línea de comando, debido a que los flujos estándares son inicializados en ese punto, en la ejecución del intérprete perl.
...
puede usarse para entrar un programa de una línea.  Si se indica B<-e>, Perl no mirará por un nombre de archivo en la lista de argumentos.  Se pueden indicar múltiples comandos B<-e> para generar un script multilínea.  Asegúrese de usar caracteres de punto y coma tal y como haría con un programa normal.
...
Si un nombre de directorio es especificado, Perl cambiará a ese directorio antes de ejecutar el programa.  La opción B<-x> controla sólo la eliminación de la basura inicial.  El programa debe terminar con C<__END__> si detrás hay basura que debe ignorarse; el programa puede procesar cualquier o toda la basura a través del identificador de archivo C<DATA>, si así se desea.
