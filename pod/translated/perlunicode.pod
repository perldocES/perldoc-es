...
Este I<pragma> no afecta a la E/S.  No cambia la representación interna de las cadenas, solo su interpretación.  Existen todavía diversos lugares dodne Unicode no se soporta completamente, como el caso de los nombres de los archivos.
...
Un patrón que se ha compilador con los modificadores C<< /u >> o C<< /a >> se trata como Unicode (aún así existen algunas restricciones con C<< /a >>).
...
Las propiedades Unicode con nombre, I<scripts>, y rangos de bloques se pueden usar (como las clases de carácter entre corchetes) usando la construcción C<\p{}> ("coincide con la propiedad") y la negación C<\P{}>, ("no coincide con la propiedad").
...
Debido a que los I<scripts> difieren en su direccionalidad (Hebrew y Arábigo se escriben de derecha a izquierda, por ejemplo) Unicode ofrece una propiedad C<Bidi_Class>.
Algunos de los valores que esta propiedad puede tener son:

    Valor       Significado

    L           Left-to-Right (de izquierda a derecha)
    LRE         Left-to-Right Embedding (de izquierda a derecha, integrando)
    ...
    RLE         Right-to-Left Embedding (de derecha a izquierda, integrando)
...
Por ejemplo, C<\p{Bidi_Class:R}> coincide con caracteres que se escriben normalmente de derecha a izquierda.  A diferencia de la propiedad C<L</General_Category>>, a esta propiedad se le puede añadir más valores en una futura distribución de Unicode.  Las listadas anteriormente comprendieron el conjunto completo para muchas distribuciones de Unicode, pero se añadieron otras en Unicode 6.3; siempre puede encontrar las actuales en L<perluniprops>.  Y L<http://www.unicode.org/reports/tr9/> describe cómo usarlas.

=head3 B<Scripts>

Las lenguas del mundo se escriben en muchos I<scripts> diferentes.  Esta sentencia (a menos que la esté leyendo traducida) está escrita en Latín, mientras que el Ruso se escribe en Cirílico, y el Griego se escribe en, bueno, Griego; el Japones, principalmente, en Hiragana o Katakana.  Hay muchos más.

El C<Script> Unicode y las propiedades C<Script_Extensions> indican en qué I<script> está un carácter dado.  Cualquier propiedad se puede especificar con la forma compuesta, como C<\p{Script=Hebrew}> (corta: C<\p{sc=hebr}>), o C<\p{Script_Extensions=Javanese}> (corta: C<\p{scx=java}>).
...
La diferencia entre estas dos propiedades implica caracteres que se usan en múltiples I<scripts>.  Por ejemplo, los dígitos '0' a '9' se usan en muchas partes del mundo.  Se colocan en un I<script> que se llama C<Common>.  Otros caracteres se usan en unos pocos I<scripts>.  Por ejemplo, el C<"KATAKANA-HIRAGANA DOUBLE HYPHEN"> se usa tanto en I<scripts> Japoneses, Katakana e Hiragana, pero en ninguno más.  La propiedad C<Script> coloca todos los caracteres que se usan en múltiples I<scripts> en el I<script> C<Common>, mientras que la propiedad C<Script_Extensions> coloca los que se usan en unos pocos I<scripts> en cada uno de aquellos; mientras, se sigue usando C<Common> para los que se usan en muchos I<scripts>.  Así, estos dos coinciden:
...
C<Script_Extensions>, entonces, es un C<Script> mejorado, en que contiene menos caracteres en el I<script> C<Common> y, correspondientemente, más en otros I<scripts>.  Es nuevo en Unicode versión 6.0, y sus datos es probable que cambien significativamente en las siguientes distribuciones, cuando las cosas se pongan en orden.
El nuevo código debería, probablemente, usar C<Script_Extensions>, y no un simple C<Script>.

(De hecho, además de C<Common>, el I<script> C<Inherited>, contiene caracteres que se usan en múltiples I<scripts>.  Existen caracteres modificadores que heredan el valor de I<script> del carácter controlador.  Algunos de estos se usan en muchos I<scripts>, y así, van en C<Inherited> tanto los que están en C<Script> como en C<Script_Extensions>.
Otros se usan en solo unos pocos I<scripts>, así que están C<Inherited> en
C<Script>, pero no en C<Script_Extensions>).

Vale acentuar que existen muchos conjuntos diferentes de dígitos en Unicode que son equivalentes a 0-9 y que coinciden con C<\d> en una expresión regular.  Si se usan en una única lengua, existen en el C<Script> y en el C<Script_Extension> de esa lengua.  Si se usan en más de un I<script>, estarán en C<sc=Common>, pero solo si se usan en muchos I<scripts>, deberían estar en C<scx=Common>.

La lista completa de I<scripts> y sus atajos está en L<perluniprops>.
...
Estos caracteres también deberían, pero no es así, afectar a C<< <> >> C<$.>, y números de líneas del I<script>.
...
En resumen, Perl trata ahora de forma normal, a los puntos de código no Unicode como puntos de código Unicode sin asignar, para las coincidencias de expresiones regulares, levantando una advertencia solo cuando el resultado sea discutible.  Aun así, si la advertencia se hace fatal, no se obvia.
...
Debido a que UTF-EBCDIC es tan similar a UTF-8, las diferencias están, en su mayor parte, escondidas; S<C<use utf8>> (y NO algo como S<C<use utfebcdic>>) declara que el I<script> está en la codificación Unicode de 8 bit "nativa" de la plataforma.  (De modo parecido para la capa C<":utf8">).
...
Cambiar el tamaño de caja de un escalar, esto es, usar C<uc()>, C<ucfirst()>, C<lc()> y C<lcfirst()>, o C<\L>, C<\U>, C<\u> y C<\l> en contextos de doble entrecomillado, como las sustituciones en expresiones regulares.
