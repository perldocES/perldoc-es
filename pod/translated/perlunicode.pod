...
Use la capa C<:encoding(...)> para leer de y escribir hacia gestores de archivos que usen la codificación que se especifica.  (Vea L<open>).
...
Este I<pragma> se creó para que pueda indicar explícitamente a Perl que las operaciones a ejecutar dentro de su ámbito deben seguir las reglas Unicode.  Los nuevos perl tendrán más operaciones afectadas.  Vea L</El "Bug Unicode">.
...
Vea L</"Propiedades de caracteres Unicode"> para más detalles.
...
Se coincide con los nombres de bloque en la forma compuesta, como C<\p{Block: Arrows}> o C<\p{Blk=Hebrew}>.  A diferencia de la mayoría de las demás propiedades, solo unos pocos nombres de bloques definidos por Unicode tienen un nombre acortado.  Peroo Perl proporciona un (leve, ya no recomendado) atajo: puede decir, por ejemplo C<\p{In_Arrows}> o C<\p{In_Hebrew}>.

Por retrocompatibilidad, el prefijo C<In> se puede omitir si no existe conflicto de nombres con un script o cualquier otra propiedad, e incluso en esos casos puede usar en su lugar un prefijo C<Is>.
Pero no haga esto en código nuevo, porque podría romperse en nuevas ediciones, algo que ya ha sucedido: Hubo una vez, en las primeras ediciones de Unicode, cuando C<\p{Hebrew}> podría coincidir con el I<bloque> Hebrew; ahora ya no.

Usando el prefijo C<In> evitamos esta ambigüedad.  Pero las nuevas versiones de Unicode continúan añadiendo propiedades cuyos nombres comienzan con C<In>.
Existe la posibilidad de que alguno de ellos, algún día, tendrá un conflicto con su trabajo.  Ya que esto es solo una extensión Perl, el nombre de Unicode tendrá precedencia y su código quedará roto.  También, Unicode es libre de añadir un script cuyo nombre comience con C<In>; eso causaría problemas.

Así que es más claro, y mejor, usar la forma compuesta cuando se especifiquen bloques.  Y asegúrese que es lo que real, realmente, quiere hacer.  En la mayor parte de los casos, los scripts es realmente lo que quiere hacer.

La lista completa de bloques y sus atajos está en L<perluniprops>.
...
=item [3] soporte no solo las listas mínimas, sino todas las propiedades de caracteres Unicode (vea Propiedades de caracteres Unicode, más arriba)
...
=item [9] Pero C<L<Unicode::LineBreak>> está disponible.

Este módulo suministra ruptura de líneas de acuerdo a L<UAX#14 "Unicode Line Breaking Algorithm"|http://www.unicode.org/reports/tr14>.
...
Nivel 3 - Soporte a medida
 ...
 [19] tiene a (?<=x) y (?=x), pero las inspecciones hacia adelante y hacia atrás
...
UTF-EBCDIC se usa en plataformas EBCDIC.  Los puntos de código Unicode toman 5 bytes para representarse (en lugar de 4 como en UTF-8), y Perl lo extiende a un máximo de 7 bytes para codificar puntos de código capaces de entrar en una palabra de 32 bit (en vez de 13 bytes y una palabra de 64 bit en UTF-8).
...
Lo siguiente son tales interfaces.  También, vea L</El "Bug Unicode">.
...
Como se ha explicado en L</Regls ASCII contra reglas Unicode>, bajo las reglas ASCII, se consideran que son caracteres sin asignar.
...
=head2 Portando código desde perl-5.6.X
...
Un gestor de archivo que lea o escriba UTF-8
...
Vea también L</El "Bug Unicode"> más arriba.
