...
automáticamente con C<use feature ':5.12'> o C<use v5.12>, o si se utiliza C<-E> en la línea de comandos de Perl 5.12 o una versión posterior.
...
Perl supone que el programa está preparado para trabajar con Unicode, pero si la codificación es otra, Perl supone que solo se desea usar ASCII por lo que los caracteres que no sean ASCII no se reconocerán como lo que serían en Unicode.
...
Sin embargo, en versiones anteriores de Perl, o si pasa cadenas a subrutinas fuera del ámbito del pragma C<feature>, puede forzar el uso de reglas Unicode cambiando la codificación a UTF-8 mediante C<utf8::upgrade($string)>. Esto se puede aplicar de manera segura a cualquier cadena, puesto que comprueba si una cadena ya se ha promovido y, en caso afirmativo, no la modifica.
...
Si no se especifica una capa de codificación, Perl intenta que cada carácter ocupe un solo byte.  Si no lo consigue, emite esta advertencia (si las advertencias están habilitadas), y utiliza datos codificados como UTF-8.
