...
En el tutorial y en las preguntas más frecuentes se supone que usa la distribución más reciente.
...
Siempre que haya intercambio de texto con alguna entidad externa al proceso de perl, como una base de datos, un archivo de texto, un I<socket> u otro programa (aunque dicha entidad externa también se haya programado en Perl).
...
Por supuesto. Si el código fuente está codificado como UTF-8, puede indicarlo con el I<pragma> C<use utf8>:
...
del ámbito del I<pragma> C<feature>, puede forzar el uso de reglas Unicode cambiando
...
=head2 ¿Y el I<pragma> C<use bytes>?
...
=head2 ¿Y el I<pragma> C<use encoding>?

No lo use. Por desgracia, este I<pragma> supone que la codificación utilizada en el entorno del programador es la misma que la utilizada en el entorno del usuario. Utilizará la misma codificación para el código fuente que para STDIN y STDOUT. Cuando se copia un programa a otro equipo, el código fuente no cambia, pero podría cambiar el entorno STDIO.

Si necesita utilizar caracteres que no pertenezcan a ASCII en el código fuente, codifíquelos como UTF-8 y use el I<pragma> C<use utf8>.

Si necesita establecer la codificación para STDIN, STDOUT y STDERR basándose, por ejemplo, en la configuración regional del usuario, use el I<pragma> C<use open>.
...
Existen algunos métodos abreviados para I<scripts> de una línea (I<one-liners>); vea la descripción del modificador L<-C|perlrun/-C [numberE<sol>list]> en L<perlrun>.
