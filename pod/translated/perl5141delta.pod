...
En este documento se describen las diferencias entre las distribuciones 5.14.0 y 5.14.1.
...
La sintaxis C<<< <<\FOO >>> en los documentos incrustados se ha documentado.
...
L<perlrun> ha experimentado una significativa limpieza.  En particular, la forma B<-0x...> de la opción B<-0> se ha clarificado, y la sección final sobre las variables de entorno ha sido corregida y ampliada.
...
Las siguientes adiciones o cambios se han realizado en la salida de diagnóstico, incluyendo advertencias y mensajes de errores fatales.  En L<perldiag> encontrará la lista completa de mensajes de diagnóstico.
...
El I<script> de prueba F<lib/locale.t> se ha actualizado para trabajar en distribución publicación del próximo Lion.
...
Se ha corregido un error que podría causar un error "uso del valor liberado en la iteración" si los próximos dos elementos hash en los que se puede iterar, se borran. [perl #85026]
...
5.14.0 introdujo algunas pérdidas de memoria en las clases de caracteres de las expresiones regulares, como C<[\w\s]>, que ahora se han corregido.
...
Varios errores de plegado de tamaño de caja Unicode se han corregido.
...
era exitoso, inapropiadamente.  Esto se ha arreglado.

=item *

Se ha corregido un caso en el que era posible que un búfer liberado podía leerse cuando era analizado un documento incrustado.
