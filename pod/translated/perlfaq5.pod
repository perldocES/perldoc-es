...
Versiones anteriores a 5.6 requiere que tenga un shell instalado que entienda las virgulillas. Versiones más tardías de Perl tienen esa característica ya incorporada. El módulo L<File::KGlob> (disponible de CPAN) proporciona una funcionalidad más portátil de glob.
...
Para solventarlo, puede tanto actualizarse a Perl v5.6.0 o superior, hacer usted mismo la expansión con readdir() y patrones, o usar un módulo como L<File::Glob>, uno que no use el shell para hacer la expansión.
...
La sentencia C<seek($gw_fh, 0, 1)> no cambia la posición actual, pero limpia la condición de fin-de-archivo en el manejador, por lo que el siguiente C<< <$gw_fh> >> hace que Perl intente de nuevo leer algo.
...
Si sigue sin funcionar, mire en el método C<clearerr> de L<IO::Handle>, que reinicia los estados de error y de fin-de-archivo en el manejador.
...
Note que "<&STDIN" hace una copia, pero "<&=STDIN" crea un alias. Eso significa que si cierra un manejador con alias, todos esos alias se volverán inaccesibles. Y esto no es verdad con una copia.
...
Ya que todas las versiones de DOS y Windows desde el MS-DOS 2.0 o así, se trata igual a los C</> y C<\> en la ruta, podría usar aquel que no choque con Perl o el shell POSIX, ANSI C y C++, awk, Tcl, Java o Python, por mencionar unos pocos. Las rutas POSIX son más portátiles, también.
