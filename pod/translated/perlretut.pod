...
C<]> es especial porque denota el final de la clase de caracteres.  C<$> es especial porque denota una variable escalar.  C<\> es especial porque se usa en las secuencias de escape, como la de más arriba.  Aquí se muestra cómo se gestionan los caracteres especiales C<]$\>:
...
=head2 Usar expresiones regulares en Perl
...
C<\Q>, C<\L>, C<\l>, C<\U>, C<\u> y C<\E> son, de hecho, parte de una sintaxis de doble entrecomillamiento, y no parte de la propia sintaxis I<regex>.  Funcionarán si aparecen en una expresión regular integradas directamente en un programa, pero no cuando están contenidas en una cadena que se interpola en un patrón.

Las I<regex> Perl pueden gestionar más que el conjunto de caracteres ASCII.  Perl soporta I<Unicode>, un estándar para representar alfabetos de, virtualmente, todos los lenguajes del mundo, y un almacén de símbolos.
...
La respuesta al requerimiento 2) es que una I<regex> (principalmente) usa caracteres Unicode.  Lo de "principalmente" es por complicadas razones de compatibilidad, pero a partir de 5.14, cualquier I<regex> compilada en el ámbito de un C<use feature 'unicode_strings'> (que se activa automáticamente dentro del ámbito de un C<use 5.012> o superior) cambiará el "principalmente" en un "siempre".  Si quiere gestionar Unicode de forma apropiada, debe asegurarse que está activada la opción C<'unicode_strings'>.
...
=head2 Compilar y guardar expresiones regulares
...
=head2 Componer expresiones regulares en tiempo de ejecución
...
=head2 Integrar comentarios y modificadores en una expresión regular
...
La primera extensión es un comentario integrado C<(?#texto)>.  Esto integra un comentario en la expresión regular sin afectar a su significado.  El comentario no debería tener ningún paréntesis de cierre, en el texto.  Un ejemplo es
...
La mayor parte de los modificadores, como C<//i>, C<//m>, C<//s> y C<//x> (o cualquier otra combinación de los mismos) pueden integrarse en una I<regex> usando C<(?i)>, C<(?m)>, C<(?s)> y C<(?x)>.  Por ejemplo,
...
Los modificadores integrados pueden tener dos importantes ventajas sobre los modificadores usuales.  Los modificadores integrados permiten aplicar un conjunto personalizado de modificadores a I<cada> patrón de la I<regex>.  Esto está muy bien para hacer coincidir un array de I<regex> que tengan diferentes modificadores:
...
La segunda ventaja es que los modificadores integrados (excepto C<//p>, que modifica la I<regex> entera) solo afectan a la I<regex> dentro del grupo en que el modificador integrado está contenido.  Así que el agrupar se puede usar para localizar los efectos de los modificadores:

    /Respuesta: ((?i)yes)/;  # coincide con 'Respuesta: yes', 'Respuesta: YES', etc.

Los modificadores integrados también pueden desactivar cualquier otro modificador activo, usando p.e., C<(?-i)>.  Los modificadores también pueden combinarse en una sola expresión, p.e., C<(?s-i)> activa el modo de solo una línea y desactiva la coincidencia de insensibilidad al tamaño de caja.

Los modificadores integrados también pueden añadirse a grupos de no captura.
...
=head2 Pragmas y depuración

Hablando de depuración, existen varios pragmas disponibles para controlar y depurar I<regex> en Perl.  Ya nos hemos encontrado con un I<pragma> en la sección anterior, S<C<use re 'eval';>>, que permite coexistir interpolación de variables y expresiones de código en una I<regex>.  Los otros pragmas son

    use re 'taint';
    $contaminado = <>;
    @partes = ($contaminado =~ /(\w+)\s+(\w+)/; # @partes está ahora contaminado

El I<pragma> C<taint> causa que cualquier subcadena de una coincidencia con una variable contaminada quede también contaminada.  Esto no es lo que normalmente sucede, ya que las I<regex> se usan a menudo para extraer partes seguras de variables contaminadas.  Use C<taint> cuando no esté extrayendo partes seguras, sino realizar algún otro procesamiento.  Tanto los pragmas C<taint> como C<eval> se aplican a ámbitos léxicos, que significa que están en efecto solo hasta el final del bloque que envuelve a esos pragmas.
...
Un método alternativo de depuración de I<regex> es el de integrar sentencias C<print>
