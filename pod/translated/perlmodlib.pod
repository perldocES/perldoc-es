...
I<Pragma> Perl para la obsolescencia de la versión principal de un módulo
...
I<socket> IP, de familia neutral, que soporta tanto IPv4 e IPv6
...
Clase base para todas las extensiones de I<shell> de CPAN
...
Utilidades para integrar Perl en aplicaciones C/C++
...
Localiza el directorio original del I<script> perl
...
Interfaz objeto para las comunicaciones con I<socket>

=item IO::Socket::INET

Interfaz de objeto para los I<sockets> del dominio AF_INET

=item IO::Socket::UNIX

Interfaz de objeto para los I<sockets> del dominio AF_UNIX
...
Códigos estándar para la identificación de I<script>
...
Imprime un mensaje de uso a partir de la documentación pod integrada
...
Clase base para los diferentes gestores de fuentes TAP
...
Definiciones de clases bases para gestores enlazados
...
Para que sea portable cada componente de un nombre de módulo debería limitarse a 11 caracteres. Si va a usarse en MS-DOS intente asegurarse de que los 8 primeros caracteres son únicos. Módulos anidados hace esto más fácil.
...
Algunas notas sobre archivos ftp: Por favor, usar un nombre de archivo largo y descriptivo, que incluya el número de versión. La mayor parte de los directorios no serán leíbles/listables, es decir, no será capaz de ver su archivo después de subirlo. Recuerde enviar su mensaje de notificación por correo electrónico tan pronto como sea posible después de subirlo, o su archivo se borrará automáticamente. Permita un tiempo para que el archivo se procese y/o compruebe antes de anunciar su ubicación.
...
=head2 Directrices para convertir bibliotecas de I<scripts> en Perl 4, a módulos
...
¡Si no está roto, no lo arregle! La biblioteca de I<scripts> de Perl 4 deben continuar funcionando sin problemas. Puede necesitar hacer algunos cambios menores (como escapar las '@' que no sean array dentro de las cadenas entrecomilladas dobles) pero no es necesario convertir un archivo .pl en un módulo solo por eso.

=item  *

Considere las implicaciones.

Todas las aplicaciones Perl que hacen uso del I<script> necesitarán cambiarse (ligeramente) si el I<script> se convierte en un módulo.  ¿No merece la pena tener un plan para hacer otros cambios, al mismo tiempo?

=item  *

Aproveche la oportunidad al máximo.

Si va a convertir el I<script> a un módulo puede aprovechar la oportunidad para rediseñar la interfaz.  Las anteriores guías para la creación de módulos incluyen muchos de los problemas que debe considerar.
