...
de los identificadores, vea L<Interpretación de identificadores>.  También es posible sustituir un identificador simple por una expresión que produzca una referencia al valor en tiempo de ejecución.   Esto se describe con mayor detalle más abajo y en L<perlref>.
...
restricciones.
Si se está trabajando bajo el efecto del I<pragma> C<use utf8;>, se aplican las siguientes reglas:
...
Si se utiliza el I<pragma> C<use warnings> o la opción de línea de comandos B<-w>, pueden aparecer advertencias sobre el uso inútil de constantes o funciones en contexto vacío ("I<void>").
...
Sin embargo, el resultado de tal partición no puede localizarse, eliminado o utilizado en una asignación.  En cambio, ahora son mucho más consistentes con las particiones hash que usan el símbolo @.
