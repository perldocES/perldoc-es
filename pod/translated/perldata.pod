...
Por otra parte, varias variables especiales que proporcionan acceso al medio interno de Perl tienen nombres que contienen caracteres de puntuación y caracteres de control.  Se describen en L<perlvar>.
...
Los nombres que comienzan con un dígito solo pueden contener otros dígitos.  Los nombres que no empiecen por una letra, un guión bajo, un dígito o el símbolo de intercalación "^" (es decir, un carácter de control) están limitados a un solo carácter, como por ejemplo C<$%> o C<$$>  (la mayor parte de estos nombres de un solo carácter tienen un significado predefinido para Perl;  por ejemplo, C<$$> es el ID del proceso actual).
...
Si no se está bajo C<use utf8>, el código fuente se trata como ASCII + 128 caracteres de control, y los identificadores tendrán que coincidir con
...
Un sigilo, seguido tanto por un circunflejo y una única letra mayúscula POSIX, como C<$^V> o C<$^W>, o un sigilo seguido por un carácter de control literal que no sea un espacio en blanco ni un C<NUL>, coincidiendo con la propiedad C<\p{POSIX_Cntrl}>.
Debido a una rareza histórica, si no se está bajo C<use utf8>, los 128 caracteres en el rango C<[0x80-0xff]> se consideran que son controles, y puede también usarse en variables de un solo carácter.  Sin embargo, el uso de caracteres no gráficos es obsoleto a partir de v5.22, y el soporte para ellos se eliminará en una futura versión de perl.  Los caracteres espacio ASCII y C<NUL> ya no están permitidos, así que eso significa que un nombre de variable de un solo carácter con ese nombre siendo un control que no sea C0 C<[0x01-0x1F]>, o C<DEL> generarán una advertencia de obsolescencia.  Realmente, bajo C<"use utf8">, los caracteres no-ASCII debe coincidir con C<Perl_XIDS>.  A parir de v5.22, cuando no se esté bajo C<"use utf8"> los controles C1 C<[0x80-0x9F]>, NO BREAK SPACE, y SOFT HYPHEN (C<SHY>)) genera una advertencia de obsolescencia.

=item *

Similar a lo anterior, un sigilo, seguido por un texto simple en llaves, donde el primer carácter es tanto un circunflejo seguido de una letra mayúscula, como C<${^GLOBAL_PHASE}> o un no-C<NUL>, un control literal no-espacio como C<${\7LOBAL_PHASE}>.  Igual que antes, cuando no se esté bajo C<"use utf8">, los caracteres en C<[0x80-0xFF]> se consideran que son caracteres de control, pero a partir de v5.22, el uso de cualquier carácter no-gráfico es obsoleto, y a partir de v5.20 el uso de cualquier carácter de control literal en el rango ASCII es obsoleto.
El soporte para estos se eliminará en una futura versión de perl.
...
Note que a partir de Perl 5.20, los caracteres de control literales en nombres de variables son obsoletos; y a partir de Perl 5.22, cualquier otro carácter no gráfico también es obsoleto.
