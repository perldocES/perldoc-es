...
un B<em> es el ancho de una "M" en la fuente de letras del documento) o en unidades aproximadamente comparables; si hay ninguna opción I<nivel de sangrado>, el valor predeterminado es cuatro.  (Y algunos formateadores puede incluso ignorar cualquier I<nivel de sangrado> indicado).  En el I<ítem> en C<=item I<ítem...>>, puede usar códigos de formateo, como se muestra aquí:
  ...
  =item Usar C<$|> para controlar el Almacenamiento temporal
...
C<for>, C<begin>, y C<end> le permitirá tener regiones de texto/código/datos que no son, generalmente, interpretados como texto Pod normal, si no que se pasan directamente a los formateadores particulares, o de alguna manera, indican que son especiales.  Un formateador que pueda usar ese formato usará esa región; sino, será completamente ignorada.
...
Enlaza a una sección en esta página del manual.  E.g.,
...
Vincule este texto a esa sección en esa página de manual.  Por ej., C<LE<lt>"if" postfijo|perlsyn/"Modificadores de instrucciones"E<gt>>

=item *

C<LE<lt>texto|/"sección"E<gt>> o C<LE<lt>texto|/secciónE<gt>> o C<LE<lt>texto|"sección"E<gt>>

Vincule este texto a esa sección en esa página de manual.  Por ej., C<LE<lt>Atributos diversos|/"Datos miembro"E<gt>>
...
Los cuatro anteriores son opcionales excepto dentro de otros códigos de formateo, notablemente con C<LE<lt>...E<gt>>, y cuándo se precede por una letra mayúscula.
...
El carácter ASCII/Latin-1/Unicode con ese número.  Si lo precede un "0x" significa que I<número> está en hexadecimal, como en C<EE<lt>0x201EE<gt>>.  Si lo precede un "0x" significa que I<número> está en hexadecimal, como en C<EE<lt>0x201EE<gt>>.  Cualquier otro I<número> se interpreta que está en decimal, como en C<EE<lt>181E<gt>>.
...
=item C<SE<lt>textoE<gt>> -- texto contiene espacio no divisible
...
Esto se ignora por la mayor parte de los formateadores, pero algunos pueden utilizarlo para generar índices.  Siempre se muestra como una cadena de caracteres vacía.
...
Un forma más legible, y quizás más "sencilla" es utilizar un conjunto alternativo de delimitadores que no requieran que se escape un simple ">".
...
De hecho, puede utilizar tantos ángulos repetidos como quiera, siempre y cuando tenga el mismo número de ellos en el delimitador de apertura y cierre, y asegurándose que hay espacio en blanco a continuación del último '<' del delimitador de apertura, e inmediatamente antes del primer '>' del delimitador de cierre.  (Se ignora el espacio en blanco).  Así que lo siguiente también funcionará: X<POD, formatting code, escaping with multiple brackets>
...
En su lugar, escriba C<la documentación LE<lt>Foo::Bar|Foo::BarE<gt>> o C<LE<lt>la documentación Foo::Bar|Foo::BarE<gt>>, para controlar el modo en que aparece el enlace.
...
L<perlpodspec>, L<perlsyn/"POD: Documentación integrada">, L<perlnewmod>, L<perldoc>, L<pod2html>, L<pod2man>, L<podchecker>.
