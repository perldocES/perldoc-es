...
Elementos de hashes restringidos (vea el pragma L<fields>) que contengan valores de copia-en-escritura no se podían eliminar, ni estos hashes podían limpiarse (C<%hash = ()>).
