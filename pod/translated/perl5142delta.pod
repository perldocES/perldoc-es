...
En este documento se describen las diferencias entre las distribuciones 5.14.1 y 5.14.2.

Si va a actualizar desde una distribución anterior, como 5.14.0, lea primero L<perl5141delta>, en el que se describen las diferencias entre 5.14.0 y 5.14.1.
...
También asegura que C<configure_requires> sólo se comprueba en los archivos META, no en los archivos MYMETA, por lo que protegen contra la generación de MYMETA que hace descartar C<configure_requires>.
...
Elementos de hashes restringidos (vea el I<pragma> L<fields>) que contengan valores de copia-en-escritura no se podían eliminar, ni estos hashes podían limpiarse (C<%hash = ()>).
...
Un pánico que implica la combinación de los modificadores de expresión regular C</aa> introducidos en 5.14.0 y la secuencia de escape C<\b>, se ha arreglado [perl #95964].
