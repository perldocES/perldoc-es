...
Este documento describe las diferencias entre las distribuciones 5.8.8 y 5.10.0.

Muchas de las correcciones de errores en 5.10.0 ya se observaban en las distribuciones de mantenimiento de 5.8.x; no se duplicarán aquí y están documentadas en el conjunto de páginas de manual llamadas perl58[1-8]?delta.

=head1 Mejoras en el núcleo principal

=head2 El I<pragma> C<feature>

El I<pragma> C<feature> se utiliza para activar una nueva sintaxis que rompe la compatibilidad Perl con distribuciones anteriores del lenguaje. Se trata de un I<pragma> léxico, al igual que C<strict> o C<warnings>.
...
El I<pragma> C<feature> es también implícitamente cargado cuando se requiere un mínimo de la versión de Perl (con la construcción C<use VERSION>) mayor que, o igual a 5.9.5. Vea L<feature> para más detalles.
...
En un ámbito donde C<$_> se ha lexicalizado, aún puede tener acceso a la versión global de C<$_> con C<$::_>, o, más sencillamente, obviando la declaración léxica con C<our $_>. (Rafael Garcia-Suarez)
...
La función Win32::GetLongPathName() ahora devuelve en codificación UTF-8 el nombre correcto del archivo largo en lugar de utilizar caracteres de reemplazo para forzar a que el nombre esté en la página de códigos ANSI.  La nueva función Win32::GetANSIPathName() se puede utilizar para convertir una ruta larga en una corta sólo si no puede representar con la página de códigos ANSI.

Muchas otras funciones en el módulo C<Win32> se han mejorado para aceptar argumentos codificados en UTF-8.  Por favor, vea L<Win32> para más detalles.
...
L<perlform> se ha mejorado, y diversos errores, corregidos.
...
La copia de la base de datos de caracteres Unicode incluidos en Perl 5 se ha actualizado a la 5.0.0.
...
C<$*>, que estaba en desuso en favor de los modificadores de exp. reg. C</s> y C</m>, se ha eliminado.
...
Se fuerza a que el identificador C<_> se considere como una palabra suelta después de un operador de test de archivo. Esto soluciona una serie de cuestiones de mala interpretación cuando esté definida una subrutina global C<_>.
...
=head2 Efecto de los I<pragma> en eval
...
Distribuciones anteriores interpretaban la palabra sencilla como un nombre de directorio.
(Gisle Aas)
...
Anteriormente, los archivos F<.pmc> sólo se cargaban si eran más recientes que el archivo F<.pm>. A partir de 5.9.4, siempre se cargarán, si existen.

=head2 $^V es ahora un objeto C<version> en lugar de un v-string

$^V puede seguir usado con el formato C<%vd> en printf, pero cualquier operación a nivel de carácter accederá a la cadena que representa el objeto C<version> y no a los ordinales del v-string.
Expresiones como C<< substr($^V, 0, 2) >> o C<< split //, $^V >> ya no funcionan y deben ser reescritas.

=head2 @- y @+ en patrones

Los arrays especiales C<@-> y C<@+> ya no se interpolan en las expresiones regulares. (Sadahiro Tomoyuki)
...
Indefinir o borrar un controlador de señales con C<undef $SIG{FOO}> es ahora equivalente a ponerlo a C<'DEFAULT'>. (Rafael Garcia-Suarez)

=head2 estado estricto y desreferencia en defined()
...
La construcción basada en la expresión regular C<(?p{})>, que fue depreciada en perl 5.8, se ha eliminado. Use en su lugar C<(??{})>. (Rafael Garcia-Suarez)

=head2 Los Pseudo-hashes han sido eliminados

Soporte para pseudo-hashes se ha eliminado a partir de Perl 5.9. (El I<pragma> C<fields> se mantiene, pero utiliza una implementación alternativa).
...
Sin embargo, el entorno de trabajo del compilador B se sigue soportando en el núcleo de perl, así como con los módulos más útiles dependientes de él (entre otro, B::Deparse y B::Concise).

=head2 Eliminación de JPL

El JPL (Java-Perl Lingo) se ha eliminado del paquete de código fuente de perl.
...
=head2 Actualizar módulos principales de forma individual

Ahora hay más módulos básicos disponibles a través de CPAN.  Si desea actualizar uno de estos módulos, no es necesario esperar a una nueva distribución de Perl.  Desde dentro de la I<shell> de cpan, ejecutando el comando 'r' presentará un informe de los módulos con actualizaciones disponibles.  Vea C<perldoc CPAN> para más información.
...
El nuevo I<pragma> C<feature> se utiliza para habilitar las nuevas características que puedan romper código antiguo. Vea L</"El I<pragma> C<feature>"> más arriba.

=item C<mro>

Este nuevo I<pragma> permite cambiar el algoritmo utilizado para resolver los métodos heredados. Vea L</"Nuevo I<pragma>, C<mro>"> más arriba.

=item Ámbito del I<pragma> C<sort>

El I<pragma> C<sort> tiene ahora un alcance léxico. Su efecto solía ser global.
...
El I<pragma> C<base> ahora avisa si una clase intenta heredar de sí misma.
(Curtis "Ovid" Poe)

=item C<strict> y C<warnings>

C<strict> y C<warnings> ahora se quejan en voz alta si se cargan usando un tamaño de caja incorrecto (como en C<use Strict;>). (Johan Vromans)
...
C<less> ahora hace algo útil (o al menos lo intenta). De hecho, se ha convertido en un I<pragma> léxico. Por lo tanto, en sus módulos, ahora puede probar si los usuarios han solicitado utilizar menos CPU, o menos memoria, menos magia, o incluso menos grasa. Vea L<less> por más. (Joshua ben Jore)
...
C<encoding::warnings>, de Audrey Tang, es un módulo para emitir avisos cada vez que una cadena de caracteres ASCII que, conteniendo bytes con los bits altos puestos, se convierte implícitamente en UTF-8. Se trata de un I<pragma> léxico desde Perl 5.9.4; en los perl más antiguos, su efecto es global.
...
C<Compress::Zlib> es un interfaz a la biblioteca de compresión zlib. Viene con una versión integrada de zlib, así que tener un zlib funcional no es un requisito previo para instalarlo. Lo utiliza C<Archive::Tar> (ver más abajo).
...
C<Params::Check> implementa un mecanismo de análisis/comprobación de entrada genérica. Lo utiliza CPANPLUS.
...
C<Log::Message> y C<Log::Message::Simple> se utilizan por la característica de registro de C<CPANPLUS>.
...
Ahora es posible acceder al I<pragma> léxico I<hints> (C<%^H>) utilizando el método B::COP::hints_hash(). Devuelve un objeto C<B::RHE>, que a su vez puede utilizarse para obtener una referencia a un hash mediante el método B::RHE::HASH(). (Joshua ben Jore)

=item C<Thread>

A medida que el viejo modelo de hilado del 5005 se ha eliminado en favor del sistema de C<ithreads>, el módulo C<Thread> es ahora un envoltorio de compatibilidad, para utilizarse solamente en el viejo código. Se ha eliminado de la lista predeterminada de extensiones dinámicas.
...
El depurador Perl ahora puede salvar todos los comandos del depurador, para listarse más tarde: notablemente, puede emular el ir hacia atrás, reiniciando y reejecutando todo hasta el último comando en el histórico de comandos almacenado.
...
Varios errores se han arreglado en C<find2perl>, con respecto a C<-exec> y C<-eval>. También han sido añadidas las opciones C<-path>, C<-ipath> e C<-iname>.
...
C<cpanp>, el I<shell> CPANPLUS, ha sido añadido. (C<cpanp-run-perl>, un ayudante para la operación con CPANPLUS, se ha añadido también, pero no está destinado para un uso directo).
...
La página de manual L<perlpragma> documenta cómo escribir su propio I<pragma> léxico en puro Perl (algo que era posible desde 5.9.4).
...
=head2 Ordenar in-situ

Ordenar arrays en el mismo sitio C<@a = sort @a> se ha optimizado para evitar crear una copia temporal del array.
...
Algunos códigos escritos en puro perl que se estaban usando para recuperar las propiedades Unicode y las asignaciones de transcripción se han reimplementado en XS.
...
En Windows, la función stat() de perl, abre, normalmente, el archivo para determinar el número de enlaces y los atributos de actualización que pueden haber cambiado a través de los enlaces duros. Poniendo ${^WIN32_SLOPPY_STAT} a un valor verdadero, aumento la velocidad de stat() al no realizar esa operación. (Jan Dubois)
...
Las alternancias, siempre que sea posible, se optimizan en estructuras de coincidencia más eficientes. Las alternancias de cadenas literales se remezclan en un árbol prefijado (Trie) y se les hace coincidir de forma simultánea.  Esto significa que en lugar de un tiempo O(N) para hacer coincidir N alternancias en un punto dado, el nuevo código lo realiza en un tiempo O(1).
Una nueva variable, ${^RE_TRIE_MAXBUF}, ha sido añadida, para ajustar esta optimización. (Yves Orton)

B<Nota:> Existe una gran cantidad de código que intenta solventar los históricos malos resultados de perl en la cuestión de las alternancias. A menudo, los trucos utilizados para esto, desactivarán las nuevas optimizaciones. Esperemos que los módulos de servicios utilizados para este propósito se aleccionarán con estas nuevas optimizaciones.

=item Optimización del punto de arranque con Aho-Corasick

Cuando un patrón comienza con una alternancia capaz de optimizarse en un árbol prefijado, y no hay mejores optimizaciones disponibles, el motor de expresiones regulares usará la coincidencia Aho-Corasick para determinar el punto de inicio. (Yves Orton)
...
Extensiones Perl en Windows pueden ahora compilarse de forma estática en la DLL de Perl.
...
Todas los archivos F<ppport.h> en los módulos XS incluidos con perl ahora se autogeneran en tiempo de generación. (Marcus Holland-Moritz)
...
La adaptación a VMS se ha mejorado. Vea L<perlvms>.
...
Los parches de fabricante se han fusionado para RedHat y Gentoo.
...
=item Llamar a CORE::require()

CORE::require() y CORE::do() siempre se interpretaban como require() y do() cuando eran sobreescritas. Esto se ha arreglado.
...
=item Sobrecargar y re-bendecir
...
Tradicionalmente, C<eval 'syntax error'> ha funcionado mal. Muchas (pero no todas) de estas fugas se han eliminado o reducido. (Dave Mitchell)
...
La variable de entorno C<PERLIO_DEBUG> ya no tiene ningún efecto para los I<scripts> setuid, ni para los I<scripts> ejecutados con B<-T>.

Además, con un perl con soporte de hilos de ejecución, usar C<PERLIO_DEBUG> podría provocar un desbordamiento de un búfer interno. Esto se ha corregido.
...
Las señales SIGILL, SIGBUS y SIGSEGV ahora siempre se entregan en una forma "insegura" (a diferencia de otras señales, que se posponen hasta que el intérprete perl llega a un estado razonablemente estable; vea L<perlipc/"Señales Diferidas (Señales Seguras)">). (Rafael)
...
=item Duplicar identificadores de archivo UTF-8
...
La advertencia I<Newline in left-justified string> se ha eliminado.

=item Too late for "-T" option

El error I<Too late for "-T" option> se ha reformulado para ser más descriptivo.
...
C<perl -V> tiene varias mejoras, por lo que es más utilizable desde I<scripts> de I<shell> para obtener el valor de las variables de configuración. Vea L<perlrun> para más detalles.

=back

=head1 Cambios internos

En general, el código fuente de perl se ha refactorizado, enlazado, y optimizado en muchos lugares. Además, la gestión de memoria y la asignación se ha mejorado en varios puntos.
...
=head2 Reordenar constantes de SVt_*

El orden relativo de las constantes que definen los diferentes tipos de C<SV> se ha cambiado; en particular, C<SVt_PVGV> ha sido movido antes de C<SVt_PVLV>, C<SVt_PVAV>, C<SVt_PVHV> y C<SVt_PVCV>.  Es poco probable que no haya ninguna diferencia a menos que tenga algún código que, explícitamente, haga suposiciones con respecto a este ordenamiento. (La jerarquía de la herencia de los objetos C<B::*> se ha cambiado para reflejar esto).

=head2 Eliminación de SVt_PVBM

Relacionado con esto, el tipo interno C<SVt_PVBM> se ha eliminado. Este tipo específico de C<SV> fue utilizado por el operador C<index> y partes del motor de expresiones regulares para facilitar las coincidencias rápidas Boyer-Moore. Su uso, internamente, ha sido sustituido por C<SV> del tipo C<SVt_PVGV>.
...
Los símbolos del preprocesador de C C<PERL_PM_APIVERSION> y C<PERL_XS_APIVERSION>, que se supone que deben almacenar el número de versión de la versión binaria más antigua de perl que es compatible a nivel binario (incluido compatible a nivel de código fuente) con la actual, no se utilizaban, y a veces tenían valores erróneos. Se han eliminado.

=head2 Menos espacio usado por las operaciones

La estructura C<BASEOP> ahora ocupa menos espacio. El campo C<op_seq> se ha eliminado y sustituido por un solo bit del campo de bits C<op_opt>. C<op_type> es ahora 9 bits de largo. (Por consiguiente, la clase C<B::OP> ya no proporciona un método C<seq>).

=head2 Nuevo analizador

El analizador de perl ahora se genera por medio de bison (que antes solía generarse por byacc). Como resultado de ello, parece ser un poco más robusto.
...
La jerarquía de la herencia de los módulos C<B::> se ha cambiado; C<B::NV> ahora hereda desde C<B::SV> (se usaba para heredar desde C<B::IV>).
...
Operadores de test de archivos apilados no funcionarán cuando el I<pragma> C<filetest> esté en vigor, porque se basan en que esté poblado el búfer C<_> por efecto de stat(), y filetest no ejecuta stat().
...
Cuando se compila con g++ y soporte para hilos en Linux, se ha informado que C<$!> deja de funcionar correctamente. Esto se relaciona con el hecho de que la glibc proporciona dos implementaciones strerror_r(3), y perl selecciona la incorrecta.
