...
=head2 How do I flush/unbuffer an output filehandle? ¿Por qué debo hacer esto?
X<flush> X<buffer> X<unbuffer> X<autoflush X<búfer>>
...
behavior within a larger program. Por ejemplo:
...
Esta subroutina agregará comas al número:
...
I<merely advisory>. Tales bloqueos, discrecionalmente, son más flexibles, pero ofrecen pocas garantías. This means that files locked with flock() may
...
=head2 I still don't get locking. I just want to increment the number in the file. ¿Cómo puedo hacer esto?
...
=head2 How can I read a single character from a file? ¿Y por el teclado?
...
to call open() should do the trick. Por ejemplo:
