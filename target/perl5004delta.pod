...
perl5004delta - qué hay de nuevo para perl5.004

=head1 DESCRIPCIÓN

Este documento describe las diferencias entre la versión 5.003 (como se documenta en I<Programming Perl>, segunda edición - el libro del Dromedario) y ésta.

=head1 Entornos soportados

Perl5.004 se compila en Unix, Plan 9, LynxOS, VMS, OS / 2, QNX, AmigaOS y Windows NT.  Perl se ejecuta en Windows 95 también, pero no se puede compilar ahí, por falta de un intérprete de comandos razonable.

=head1 Cambios principales

Más importante aún, muchos errores han sido solucionados, incluyendo varios problemas de seguridad.  Ver el archivo F<Changes> en la distribución para más detalles.

=head2 La asignación de lista a %ENV funciona

C<%ENV = ()> y C<%ENV = @list> ahora funcionan como se esperaba (excepto sobre VMS donde genera un error fatal).

=head2 Cambiado el error "Can't locate Foo.pm in @INC"

El error "Can't locate Foo.pm in @INC" ahora lista los contenidos de @INC para una depuración más sencilla
...
probablemente funcionará para un script ejecutable invocado como C<nombredelscript>, mientras que:
...
=head2 Advertencias más precisas
...
compiled with -DPERL_EMERGENCY_SBRK and used Perl's malloc.  Entonces
...
The new conversions in Perl's sprintf() are:
   ...
   %n	special: *stores* the number of characters output so far
        en la siguiente variable en la lista de parámetros 
...
allocated for the given hash.  Puede hacerle ganar más eficiencia si sabe que el hash va a tener un gran tamaño.  (Esto es similar a pre-extender un array asignándole un gran número a $#array).  Si dice
...
No puede reducir el número de casillas reservados para el hash usando C<keys> de esta manera (pero no necesita preocuparse si hace esto por accidente; intentarlo no tiene ningún efecto).
...
Si el argumento VERSIÓN está presente entre el Módulo y la LISTA, entonces C<use> llamará al método VERSIÓN en la clase Módulo con la versión dada como argumento.  The default VERSION method, inherited from
...
Devuelve el prototipo de una función como una cadena (o C<undef> si la función no tiene ningún prototipo).  FUNCTION is a reference to or the name of the
...
allows the ability to check what a reference points to. Ejemplo:
...
object-oriented overrides.  Son los siguientes:
...
(F) The given character is not a valid pack type.  Vea L<perlfunc/pack>.

=item Invalid type in unpack: '%s'

(F) The given character is not a valid unpack type.  Vea L<perlfunc/unpack>.
...
the outermost subroutine.  Por ejemplo:
...
Si usted cree que tiene un error no declarado, por favor, ejecute el programa B<perlbug> incluido en su distribución.  Make sure you trim your bug down
...
El archivo F<INSTALL> contiene instrucciones para compilar Perl.  This file has been
