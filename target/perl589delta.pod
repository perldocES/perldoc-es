...
C<system> operator. Vea L<perlvar> para más detalles. (Escrito por Gisle Aas).
...
C<Module::Pluggable> es un simple marco de trabajo para crear módulos conectables que aceptan submódulos enchufables. The bundled version is 3.8
...
when they were overridden. Esto ha sido arreglado.
...
C<-foo=bar> with the C<-s> on the <#!> line. Esto ha sido corregido. See
...
Muchos módulos actualizados desde CPAN incorporan nuevas pruebas. Some core specific
...
attack on 25th September 2006. Le echaremos de menos.
