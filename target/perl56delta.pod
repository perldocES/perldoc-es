...
perl56delta - qué hay de nuevo para perl v5.6.0

=head1 DESCRIPCIÓN

Este documento describe las diferencias entre la versión 5.005 y la versión 5.6.0.
...
Por ejemplo, C<"¡Hola! \N{WHITE SMILING FACE}"> evalúa a una cadena con una cara sonriente unicode.
...
of characters with the specified ordinals.  Esta es una forma alternativa, más fácil de leer para la construcción de cadenas (posiblemente unicode) en lugar de interpolar caracteres, como en C<"\x{1}\x{2}\x{3}\x{4}">.  The leading
...
C<require> y C<use> también tienen algo de magia especial para soportar tales literales, pero este uso particular debe ser evitado, ya que conduce a mensajes de error en las versiones de Perl que no soportan cadenas de vectores.  Usando un verdadero número de versión garantizará un comportamiento correcto en todas las versiones de Perl:

    require 5.006;    # verificación en tiempo de ejecución para v5.6
    use 5.006_001;    # verificación en tiempo de compilación para v5.6.1
...
=head2 @- y @+ ofrecen desplazamientos de los comienzos y finales de las concordancias en las expresiones regulares
...
Vea L<perlguts/Antecedentes y PERL_IMPLICIT_CONTEXT> para consultar información detallada sobre las consecuencias de la compilación de Perl con esta opción.
...
=head2 Fallos de test en los hilos de ejecución
...
=head2 Plataformas EBCDIC no soportadas
...
=head2 En HP-UX 64-bit, el test lib/io_multihomed puede colgarse
...
=head2 Fallo de test en el POSIX de NEXTSTEP 3.3
...
=head2 Fallo del test lib/sdbm con el gcc en Tru64 (alias Digital UNIX, alias DEC OSF/1)
