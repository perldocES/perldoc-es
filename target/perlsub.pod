...
Para declarar subroutinas:
X<subroutine, declaration> X<sub>

    sub NOMBRE;			  # una "breve" declaración.
...
X<subroutine, anonymous>
    ...
    $subref = sub BLOQUE;		 # no utiliza prototipos 
    $subref = sub (PROTO) BLOQUE;	 # con prototipo 
    $subref = sub SIG BLOCK;             # with signature
    $subref = sub : ATTRS BLOQUE;	 # con atributos
    $subref = sub (PROTO) : ATTRS BLOQUE; # con prototipo y atributos
    $subref = sub : ATTRS SIG BLOCK;     # with attribs and signature

Para importar subrutinas:
X<import>

    use MODULE qw(NOMBRE1 NOMBRE2 NOMBRE3);

Para invocar subrutinas:
X<subroutine, call> X<call>

    NOMBRE(LIST);	   # & es opcional con los paréntesis.
    NOMBRE LIST;	   # Los paréntesis son opcionales si fue importada.
    &NOMBRE(LIST);   # evita prototipos.
    &NOMBRE;	   # Hace visible la variable @_ para la subrutina invocada.

=head1 DESCRIPCIÓN

Como muchos lenguajes, Perl provee subrutinas definas por el usuario.
Estas pueden estar ubicadas en cualquier parte del programa principal, cargadas por otros archivos a través de las palabras claves C<do>, C<require>, o C<use>, o
generados en ejecución usando C<eval> o subrutinas anónimas.
Puedes incluso invocar indirectamente a una función mediante una variable que contiene su nombre o una referencia.

El modelo de perl para invocar una función y retornar valores es simple: todas las funciones reciben como parámetros una lista plana de escalares, e igualmente para todas las funciones se retorna a su invocador una lista plana de escalares.  Any arrays or hashes in these call and return lists will
collapse, losing their identities--but you may always use
pass-by-reference instead to avoid this.  Tanto las listas al invocar y las listas que retornan pueden contener muchos o pocos elementos escalares como desees.  (A menudo
una función sin una sentencia return explícita es llamada subrutina, pero realmente no hay ninguna diferencia desde la perspectiva de Perl). 
X<subrutina, parametro> X<parametro>

Cualquier argumento pasado como parámetro de entrada es mostrado en el array C<@_>.
(They may also show up in lexical variables introduced by a signature;
see L</Signatures> below.)  Entonces, si
invocaste una función con dos argumentos, estos podrían ser almacenados en
C<$_[0]> y C<$_[1]>.  El array C<@_> es un array local, pero sus elementos son alias para los parámetros escalares reales.  En particular,
si un elemento C<$_[0]> es actualizado, el argumento correspondiente es actualizado (o puede ocurrir un error si no es actualizable).  Si un argumento es un array o un elemento hash que no existía cuando la función fue invocada, ese elemento es creado solo cuando (y si) es modificado o es utilizado mediante una referencia a él.  (Algunas versiones anteriores de Perl crean el elemento así tenga o no valor asignado.)
Asignando por completo el array C<@_> elimina este alias, y no actualiza ningún argumento.
...
context, or nothing in void context.  Si retornas uno o más
agregados (arrays y hashes), estos serán almacenados de forma indistinguible en una gran lista.

Si C<return> no es encontrado y la última sentencia es una expresión, se
devuelve su valor. Si la última sentencia es una estructura de control de ciclo
como un C<foreach> o un C<while>, El valor devuelto es indefinido. Una
subrutina vacía retorna una lista vacía.
X <subroutine, return value> X <return value> X <return>

Aside from an experimental facility (see L</Signatures> below),
Perl does not have named formal parameters.  En la práctica todo lo que haces es asignar a un C <my()> lista de éstos.  Las variables que no son
declaradas como privadas son variables globales.  Para más detalles escabrosos
sobre la creación de variables privadas, ver L<"Variables Privadas vía my()">
y L<"Valores Temporales vía local()">.  Para crear ambientes protegidos para un conjunto de funciones en un paquete separado (y probablemente en un archivo separado), véase L<perlmod/"Packages">.
...
Ejemplo:
    ...
    # obtén una línea, combinando líneas continuas
    ...
    $lookahead = <STDIN>;	# obtén la primera línea
...
de convertir una llamada-por-referencia en una llamada-por-valor.  Además una función es libre de hacer modificaciones de C<@_> y cambiar los valores del invocador.
...
Por supuesto, no está permitido modificar constantes de esta manera.  Si un
argumento era en realidad literal y trataste de cambiarlo, deberías obtener una
(presumiblemente fatal) excepción.   Por ejemplo, esto no funciona:
X<call-by-reference> X<call-by-value>

    upcase_in("frederick");

Sería mucho más seguro si la función C<upcase_in()> fuera escrito para devolver una copia de sus parámetros en lugar de cambiarlos:

    ($v3, $v4) = upcase($v1, $v2);  # esto no cambia $v1 y $v2
    sub upcase {
	return unless defined wantarray;  # contexto void, nada que hacer
	my @parms = @_;
	$_ =~ tr/a-z/A-Z/
  	return wantarray ? @parms : $parms[0];
    }

Observe cómo esta función (sin prototipo) no le importa si fueron pasados escalares o arrays.  Perl considera todos los argumentos como uno grande, una larga lista, de parámetros aplanados en C<@_>.  Esta es un área donde la simplicidad de Perl en el estilo de pasaje de argumentos brilla.  La función C<upcase()>
podría funcionar perfectamente bien sin cambiar la definición C<upcase()>
inclusive si nosotros alimentamos cosas como estas:

    @newlist   = upcase(@list1, @list2);
    @newlist   = upcase( split /:/, $var );

No obstante, la tentación de hacer esto:

    (@a, @b)   = upcase(@list1, @list2);

Al igual que la lista de parámetros de entrada aplanada, la lista de retorno también está aplanado en el retorno.  Así que todo lo que han logrado hacer aquí es almacenar todo en C<@a> y dejar C<@b> vacío.  Ver L<Pass by Reference> alternativas.

Una subrutina puede ser llamada con un prefijo explícito C<&>.  El C<&> es opcional en el Perl moderno, así como los paréntesis si la subrutina ha sido predeclarada.  El C<&> I<no> es opcional
cuando sólo estoy nombrando a la subrutina, como cuando es usado como un argumento para defined() o undef().  Tampoco es opcional cuando quieres hacer una llamada indirecta a una subrutina con un nombre de subrutina o usando una referencia mediante el C<&$subref()> o el constructor C<&{$subref}()>, aunque la notación de la siguiente expresión C<< $subref->() >>resuelve el problema.
Ver también L<perlref>.
X<&>

Las subrutinas pueden ser llamadas de forma recursiva.  Si una subrutina es llamada
usando la forma C<&>, la lista de argumentos es opcional, y si es omitida,
el array C<@_> no se crea para la subrutina: en su lugar el array C<@_> en el tiempo de llamada es visible a la subrutina.  Este es un mecanismo eficiente que los nuevos usuarios pueden desear evitar.
X<recursion>
    ...
    &foo(); # igual al anterior

    &foo; # foo () recibe argumentos actuales, como foo(@_)!!
    foo;		# like foo() IFF sub foo predeclared, else "foo"

No sólo la forma C<&> hace que la lista de argumentos sean opcionales, También de esta forma se deshabilita cualquier validación del prototipo sobre los argumentos que proporcionas.  Esto es en parte por razones históricas, y en parte para tener una manera conveniente de hacer trampa si sabes lo que estás haciendo.  Ver L</Prototypes> a continuación.
X<&>

Desde Perl 5.16.0, el I<token> C<__SUB__> esta disponible bajo la sentencia C<use feature
'current_sub'> y C<use 5.16.0>.  Esto evaluará con una referencia a la ejecución actual de la subrutina, lo que permite llamadas recursivas sin conocer el nombre de la subrutina.
...
Subrutinas cuyos nombres están en mayúsculas se reservan para el núcleo de Perl, así como los módulos cuyos nombres están en minúsculas.  Una subrutina capitalizada es una convención que indica que será invocado
indirectamente por el sistema en tiempo de ejecución, por lo general debido a un evento activado.
...
Las subrutinas C<BEGIN>, C<UNITCHECK>, C<CHECK>, C<INIT> y C<END>
no son enteramente subroutinas aunque se nombran como bloques de código especial, de los
cuales puede tener más de uno en un paquete, y los puedes B<not> invocar
explícitamente.  See L<perlmod/"BEGIN, UNITCHECK, CHECK, INIT and END">
...
a parameter declaration, leaving just a bare C<$> sigil.  Por ejemplo,
...
one parameter vary according to the earlier parameters.  Por ejemplo,
...
L<prototype attribute|attributes/Built-in Attributes>.  Por ejemplo,
...
SINOPSIS
    ...
    my @oof = @bar;	# declara @oof localmente, y lo inicializa
    my $x : Foo = $y;	# similar, con un atributo aplicado applied

B<ADVERTENCIA>: El uso de listas de atributos sobre la declaración C<my>  todavía está evolución.  La actual semántica e interfaces están sujetas a cambios.
Ver L<attributes> y L<Attribute::Handlers>.

El operador C<my> declara las variables listadas a ser lexicalmente confinadas al bloque que las contienen, los condicionales (C<if/unless/elsif/else>),
bucles (C<for/foreach/while/until/continue>), subrutinas, C<eval>,
o C<do/require/use> archivos.  Si se indica más de un valor, esa lista debe ser puesta entre paréntesis.  Todos los elementos mencionados deben ser valores-izquierda legales.  Solo identificadores alfanuméricos pueden ser de ámbito léxico--en su lugar los operadores mágicos predefinidos como C<$/>deben ser C<local>izados con C<local>.
...
change whether those variables are viewed as a scalar or an array.  Así que
...
the current statement.  Por lo tanto,
...
as a scalar or an array.  Así que
...
mechanism is generally easier to work with.  Vea más abajo.
...
whatever C<*> value was assigned to it.  Ejemplo:
...
allowed argument type. Por ejemplo:
...
the argument C<"Foo/Bar.pm"> in @_.  Vea L<perlfunc/require>.
