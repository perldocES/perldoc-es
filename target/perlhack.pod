...
En este documento se explica cómo funciona el desarrollo de Perl.  It includes details
...
=head1 GUÍA ULTRARRÁPIDA DE CREACIÓN DE REVISIONES

Si desea enviar un único archivo de parcheo pequeño, como una corrección de un documento pod, un test de un error, correcciones de comentarios, etc., el procedimiento es muy sencillo. Here's how:
...
Puede ejecutar todos los tests con los siguientes comandos:
...
sentence.  Por ejemplo, "Corrección de errores ortográficos en perlhack.pod".

=item * Envíe el cambio a perlbug

El siguiente paso es enviar el parche al sistema de tiques del núcleo de Perl, vía correo electrónico.
...
Puede revisar los errores y los parches de Perl existentes a través de la interfaz web L<http://rt.perl.org/>.
...
La lista de correo perl5-changes recibe una copia de cada parche enviada a las ramas de mantenimiento y desarrollo del repositorio de perl.  See L<http://lists.perl.org/list/perl5-changes.html> for
...
Todo el código fuente de Perl se almacena en un repositorio central de Git en I<perl5.git.perl.org>.  The repository contains many Perl revisions
...
Puede tener acceso al repositorio a través de Internet.  This allows you to browse
the tree, see recent commits, subscribe to RSS feeds for the changes,
search for particular commits and more.  Se encuentra en L<http://perl5.git.perl.org/perl.git>.  A mirror of the repository is
...
=head2 Envío de parches

If you have a small patch to submit, please submit it via perlbug.  También puede enviar un mensaje de correo directamente a perlbug@perl.org.  Tenga en cuenta que los mensajes enviados a través de perlbug se canalizarán a través de una cola de moderador///, por lo que es posible que no reciba una respuesta inmediatamente.
...
Patches are reviewed and discussed on the p5p list.  En general, as revisiones sencillas y que no generen controversia, se aplicarán sin debate.
Cuando se haya aplicado el parche, se actualizarán la incidencia y se le enviará una notificación por correo electrónico.  También se enviará un mensaje de correo electrónico a la lista p5p.
...
Los cambios siempre se aplican directamente a la rama de desarrollo principal,
denominada "blead".  Some patches may be backported to a maintenance
...
Este comando genera un parche basado en las diferencias entre la rama blead y su rama actual.  Es importante asegurarse de que la rama blead está actualizada antes de generar el archivo de diferencias, por lo que antes hay que ejecutar el comando C<git pull>.

Es muy recomendable usar git (si es posible).  It will make your
life easier, and ours as well.

Sin embargo, puede crear un parche válido aunque no use git.  You'll need a pristine copy of the Perl source to diff against.
The porters prefer unified diffs.  Puede usar GNU C<diff> para producir un archivo de diferencias:
...
you're fixing or validate the new functionality you're adding.  En general, debe actualizar un archivo de test existente en lugar de crear uno nuevo.
...
Know what you're testing.  Lea la documentación y el código fuente.
...
La distribución principal ahora puede ejecutar sus pruebas de regresión en paralelo en las plataformas de tipo Unix.  Instead of running C<make test>, set C<TEST_JOBS>
in your environment to the number of tests to run in parallel, and run
C<make test_harness>.  En un shell de la familia Bourne, esto se puede hacer así
...
=head2 Ejecutar tests manualmente
...
(Si no especifica scripts de prueba, se ejecutar todo el conjunto de pruebas.)
...
some tests that interact with the outside world are skipped.  Consulte L<perl58delta>.
