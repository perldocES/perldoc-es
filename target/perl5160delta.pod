...
existing breakpoints and re-enabling them.  Vea L<perldebug>.
...
pointers to null.  Error reportado por Clément Lecigne. (5.14.2)
...
Se corrige un problema con C<< open my $fh, ">", \$escalar >>, al no funciona si C<$escalar> es un escalar con copia-en-escritura. (5.14.2)
...
L<perlrun> ha experimentado una significativa limpieza.  Most notably, the
...
Los mensajes de error por usar C<default> y C<when> fuera de un actualizador han sido estandarizados para que coincidan con los mensajes de C<continue> y controles de bucle.  Ahora dicen 'I<Can't "default" outside a topicalizar>' ('No se puede usar "default" fuera de un actualizador') y 'I<Can't "when" outside a topicalizar>' ('No se puede usar "when" fuera de un actualizador').  Los dos eran 'I<Can't use when() outside a topicalizer>' ('No se puede usar when() fuera de un actualizador') [perl #91514].
...
Una solución para corregir el tamaño del I<socket> hace que se pase sin problemas las pruebas en HP-UX PA-RISC para las versiones de 64bitall. (5.14.2)
...
El primero es siempre 0, los otros dos ahora son no-ops. (5.14.1)
...
C<utf8::decode> tuvo un desagradable error que modificaría, en el mismo sitio, escalares copia-en-escritura que sean tampones de cadenas, (es decir, no haría la copia).  Esto permitía que un hash tuviera dos elementos con la misma clave [perl #91834]. (5.14.2)
...
Elementos de hashes restringidos (vea el pragma L<fields>) que contengan valores de copia-en-escritura no se pudo eliminar, ni estos hashes podían ser limpiados (C<%hash = ()>). (5.14.2)
...
specific scalar return by C<undef()> (C<&PL_sv_undef> internally).  Esto ha sido corregido.  C<undef()> is now treated like other undefined
...
5.14.0 introdujo algunas pérdidas de memoria en las clases de caracteres de las expresiones regulares, como C<[\w\s]>, que ahora han sido corregidas. (5.14.1)
...
es uno de esos casos.  C<\xDF> se pliega en C<"ss">. (5.14.1)
...
mischief would take place.  Esto ha sido corregido.
...
was not a default warning.  Esto ha sido corregido.
...
apply to fully-qualified bareword handle names not followed by C<||>.  Esto ha sido corregido.
...
Perl 5.10.0 introdujo una lógica defectuosa que hizo que "U*", en el medio de una plantilla pack, equivalga a "U0" si la cadena de entrada estaba vacía.  Esto se ha corregido [perl #90160]. (5.14.2)
...
C<caller> ya no agota la memoria cuando se le llama desde un paquete DB si C<@DB::args> fue asignada después de la primera llamada a C<caller>.  L<Carp> desencadenaba este error [perl #97010]. (5.14.2)
...
Si el fallo que está reportando tiene implicaciones de seguridad, que lo hace inadecuado para ser enviado a una lista de correo pública, por favor, envíelo a perl5-security-report@perl.org.  This points to a closed
