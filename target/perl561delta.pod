...
perl561delta - qué hay de nuevo para perl v5.6.1

=head1 DESCRIPCIÓN

Este documento describe las diferencias entre la versión 5.005 y la versión 5.6.1.

=head1 Resumen de cambios entre 5.6.0 y 5.6.1

Esta sección contiene un resumen de los cambios entre la versión 5.6.0 y la versión 5.6.1.  Más detalles acerca de los cambios mencionados aquí se pueden encontrar en el archivo F<Changes> que acompaña a la distribución de código fuente Perl.  Vea L<perlhack> para enlaces a recursos en línea donde puede inspeccionar los parches individuales descritos por estos cambios.

=head2 problemas de seguridad

suidperl no ya ejecutará /bin/mail, porque en algunas plataformas /bin/mail es vulnerable a ataques de desbordamiento de búfer.

Tenga en cuenta que suidperl no es compilado ni instalado por defecto en ninguna versión reciente de perl.  El uso de suidperl está sumamente desaconsejado.
Si cree que lo va a necesitar, pruebe en primer lugar otras alternativas tales como sudo.
Vea http://www.courtesan.com/sudo/ .

=head2 Correcciones de errores principales

Esta no es una lista exhaustiva.  Se pretende cubrir únicamente los cambios importantes visibles por el usuario.

=over

=item C<UNIVERSAL::isa()>

Un error en el mecanismo de caché utilizado por C<UNIVERSAL::isa()> que afecta a base.pm ha sido corregido.  El error ha existido desde las versiones 5.005, pero hasta ahora no le habían hecho cosquillas a base.pm.

=item Pérdidas de memoria

Varios de los casos de pérdidas de memoria y los intentos de acceso a memoria no inicializada han sido curados.  Vea L</"Problemas conocidos"> a continuación para otras cuestiones.

=item Conversiones numéricas

Las conversiones numéricas no reconocen cambios en el valor de la cadena de caracteres de forma correcta, en determinadas circunstancias.
...
=item Errores en expresiones regulares
...
Multi-line matches like C<"a\nxb\n" =~ /(?!\A)x/m> were flawed.  Este error ha sido solucionado.

Use of $& could trigger a core dump under some situations.  Ahora se evita todo esto.
...
the end in certain situations.  Esto ha sido corregido.
...
in L<perlvar> (as in C<${$num}>) was accidentally disabled.  Esto ahora funciona de nuevo.

=item Lexical warnings 

Lexical warnings now propagate correctly into C<eval "...">.

C<use warnings qw(FATAL all)> did not work as intended.  Esto ha sido corregido.
...
when statically building extensions into perl.  Esto ha sido corregido.

"our" variables could result in bogus "Variable will not stay shared"
warnings.  Esto ha sido arreglado.
...
Perl 5.6.0.  Esto ha sido corregido.
...
in a directory other than the perl source directory.  Vea F<INSTALL>.
...
isn't running.  Esto ha sido corregido.
...
Trailing new %ENV entries weren't propagated to child processes.  Esto ha sido arreglado.
...
=head2 Clonado del intérprete, hebras, y concurrencia
...
interpreter level.  Vea L<overload> para los detalles sobre esto.

Esta característica está aún en evolución.  It is eventually meant to be used
...
support for running B<cloned> interpreters concurrently.
    ...
    NOTE: Esta característica es experimental.  Los detalles de la implementación está
    sujeto a cambios.
...
will be needed to complete the toolkit for dealing with Unicode.
    ...
    NOTE: This should be considered an experimental feature.  Implementación
    , sus detalles están sujetos a cambios.

=head2 Support for interpolating named characters

The new C<\N> escape interpolates named characters within strings.
Por ejemplo, C<"¡Hola! \N{WHITE SMILING FACE}"> evaluates to a string
with a Unicode smiley face at the end.

=head2 Declaraciones "our"
...
with the stubs they provide.  Vea L<attributes>.
...
of the traditional two-argument form.  Vea L<perlfunc/open>.

=head2 Soporte para 64 bit
...
and compile Perl using the -Duse64bitint Configure flag.
    ...
    depreciado.  Use -Duse64bitint instead.
...
start losing precision (in their lower digits).
    ...
    Existing support only covers the LP64 data model.  En particular, el
    LLP64 data model is not yet supported.  64-bit libraries and system
    APIs on many platforms have not stabilized--your mileage may vary.

=head2 Soporte para archivos grandes
...
=head2 Dobles largos (punto flotante)
...
be compared are passed as normal parameters in @_.  Vea L<perlfunc/sort>.
...
function in earlier versions.  Esto ahora está permitido.
...
Vea L<perlre> para más detalles.
...
contains additional documentation.
    ...
    NOTE: Esta característica es experimental.  Los detalles están sujetos a cambios.  

=head2 Soporte para números binarios

Binary numbers are now supported as literals, in s?printf formats, and
C<oct()>:

    $respuesta = 0b101010;
    printf "La respuesta es: %b\n", oct("0b101010");
...
Vea L<perlfunc/exists> para más ejemplos.
...
such as C<< $ph->{foo}[1] >>, was accidentally disallowed.  Esto ha sido corregido.
...
fields::new() and fields::phash().  Vea L<fields>.
...
Vea L<perlfunc/"binmode"> y L<open>.
...
number and the line number within the evaluated text itself.  Por ejemplo:
...
argument, making the cases shown above syntax errors.  El comportamiento habitual de:

    print defined &foo, &bar, &baz;
    print uc "foo", "bar", "baz";
    undef $foo, &bar;

se mantiene sin cambios.  Vea L<perlop>.
...
=head2 Características de seguridad mejoradas
...
La vieja sintaxis no ha cambiado.  As before, `^X' may be either a
...
only during normal running are warranted.  Vea L<perlvar>.
...
go to achieve production quality compiled executables.
    ...
    NOTE: The Compiler suite remains highly experimental.  La versión definida es simplemente una cadena de
...
means "for at least 3 CPU seconds".  El formato de salida también ha cambiado.  Por ejemplo:
...
Para otros detalles, vea L<Benchmark>.

=item ByteLoader

The ByteLoader is a dedicated extension to generate and run
Perl bytecode.  Vea L<ByteLoader>.
...
This pragma implements the C<\N> string escape.  Vea L<charnames>.

=item Data::Dumper

A C<Maxdepth> setting can be specified to avoid venturing
too deeply into deep data structures.  Vea L<Data::Dumper>.
...
Vea C<ext/DB_File/Changes>.

=item Devel::DProf

Devel::DProf, a Perl source code profiler has been added.  Vea L<Devel::DProf> y L<dprofpp>.
...
comparison functions.  Vea L<File::Compare>.
...
to the File::Spec module.  Permite abreviar

    $nombrecompleto = catfile($dir1, $dir2, $archivo);

en lugar de

    $nombrecompleto = File::Spec->catfile($dir1, $dir2, $archivo);
...
Vea L<Pod::Usage> para más detalles.
...
not complete yet.  Vea L<Pod::Checker>.
...
preferred interface.  Vea L<Pod::Text> para más detalles.  The new Pod::Text
...
=head1 Mejoras en la Documentación

Many of the platform-specific README files are now part of the perl
installation.  Vea L<perl> para ver la lista completa.
...
=head2 Nuevas banderas para Configure
...
=head1 Cambios específicos de cada plataforma

=head2 Plataformas soportadas
...
The builtin XSUB functions in the Win32:: namespace are documented.  Vea L<Win32>.

$^X now contains the full path name of the running executable.

A Win32::GetLongPathName() function is provided to complement
Win32::GetFullPathName() and Win32::GetShortPathName().  Vea L<Win32>.
...
to do nothing):
    ...
    perl -0777 -pi -e 's/^/foo/' archivo_vacío

El comportamiento de:

    perl -pi -e 's/^/foo/' archivo_vacío

is unchanged (it continues to leave the file empty).

=head2 Mejoras en C<eval '...'>
...
correctly when no exception occurred within the eval.  Esto ha sido corregido.

Parsing of here documents used to be flawed when they appeared as
the replacement expression in C<eval 's/.../.../e'>.  Esto ha sido corregido.
...
inadvertently set $? o $!.  Esto ha sido corregido.
...
the original list was empty.  Considere el siguiente ejemplo:
...
back to the default "C" locale.  Esto ha sido corregido.
...
those numbers produced correct results.  Estas advertencias han sido descontinuadas.

=head2 Pérdidas de memoria

The C<eval 'return sub {...}'> construct could sometimes leak
memory.  Esto ha sido corregido.

Operations that aren't filehandle constructors used to leak memory
when used on invalid filehandles.  Esto ha sido corregido.

Constructs that modified C<@_> could fail to deallocate values
in C<@_> and thus leak memory.  Esto ha sido corregido.
...
cause silent failures.  Esto ha sido corregido.
...
(F) Lexically scoped subroutines are not yet implemented.  No intente hacer esto (aún).
...
an ampersand before the name to avoid the warning.  Vea L<perlsub>.
...
=item Can't declare %s in "%s" (No se puede declarar %s en "%s")
...
C<overload> or C<charnames> pragma?  Vea L<charnames> y L<overload>.
...
you intended only to read from the file, use "<".  Vea L<perlfunc/open>.
...
used to separate keys from values.  El elemento se ignora.
...
by Perl or by a user-supplied handler.  Vea L<attributes>.
...
your signed integers.  Vea L<perlfunc/pack>.

=item Repeat count in unpack overflows

(F) You can't specify a repeat count so large that it overflows
your signed integers.  Vea L<perlfunc/unpack>.
...
character to get your parentheses to balance.  Vea L<attributes>.
...
=item Número de versió debe ser un número constante
...
=head2 Incompatibilidades en el código fuente Perl
...
=head2 Incompatibilidades en el código fuente C
...
on the ramifications of building Perl with this option.
    ...
    with one of -Dusethreads, -Dusemultiplicity, or both.  No se
...
=head2 Incompatibilidades binarias
...
=head2 Fallos conocidos en las pruebas
...
operation must be considered erroneous. Por ejemplo:
...
=head2 Características experimentales
...
even subject to removal in some future release of Perl.  Estas características son las siguientes:
...
way to do it with multiple statements.  Vea L<perlre>.
...
También puede haber información en http://www.perl.com/, la página oficial de Perl.
...
Escrito por Gurusamy Sarathy <F<gsar@activestate.com>>, con muchas contribuciones de los I<Perl Porters>.
