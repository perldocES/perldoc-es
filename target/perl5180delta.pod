...
ignored when interpreting a string as a number.  Por ejemplo:
...
while anonymous subs are created at run-time.)  Por ejemplo,
...
using xlC.  Esto lo hará más fácil compilar un cierto número de módulos basados en XS que asumen la presencia de C99 [perl #113778].
...
required for the correct behavior of closures. Por ejemplo:
...
as C<(?:)> or C<(?:|)>) could disable some optimizations. Esto ha sido corregido.
...
somewhere.  Le echaremos de menos.
