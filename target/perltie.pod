...
(probably anonymous) that it's creating.  Por ejemplo:
...
size, you'll take an exception.  Por ejemplo:
...
object I<this>. (Equivalent to C<scalar(@array)>).  Por ejemplo:
...
Append elements of I<LIST> to the array.  Por ejemplo:
...
Remove last element of the array and return it.  Por ejemplo:
...
each() function to iterate over such.  Ejemplo:
...
Hasta aquí todo bien.  Those of you who have been paying attention will have
...
No es fácil atar una estructura de datos de múltiples niveles (por ejemplo, un hash de hashes) a un archivo dbm.  El primer problem es que todas, excepto las BD GDBM y Berkeley tienen limitaciones de tamaño; pero aparte de eso, también tiene problemas con cómo las referencias son representadas en disco.  One
