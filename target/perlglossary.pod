...
a holding area. Esto puede ser muy ineficaz.
...
the function-call mechanism. (En realidad, los programadores se pasaron a C porque les gustaban más los identificadores en minúsculas que en mayúsculas). Perl is
...
(including newlines) is gleefully ignored. Casi siempre.
...
definitions. También se denomina I<cpp>(1).
...
Derived X<eclectic (term)>from many sources. Hay quien piense que de I<demasiadas>.
...
B<process> inherits from its parent. Se puede acceder a estas variables a través de C<%ENV>.
...
Final del archivo. X<End of File (EOF)>X<EOF (End of File)>Sometimes used
...
Término pretencioso para designar un error. See B<fatal error>.
...
office metaphors. Los sistemas de archivos modernos permiten asignar varios nombres a un archivo. Some files have special properties, like directories and
devices.
...
it can’t get away with using B<integers>. Los números de punto flotante son meras aproximaciones de los números reales.
...
things tidy. Sin embargo, raramente hablamos en un sentido estricto y nos referiremos habitualmente al esquema de recuento de referencias como una forma de recolección de elementos no deseados. (If it’s any
...
B<interpreter> is being shut down. La destrucción global no debe confundirse con el Apocalipsis, salvo cuando se refiera a eso explícitamente.
...
certain obscure operations. Los archivos de encabezado de C tienen la extensión I<.h>. Perl
...
En Perl, las constantes hexadecimales empiezan por C<0x>. See also the C<hex>
...
actually anticipate them. O que al menos tratarán de hacerlo. Es por ello la segunda gran virtud de un programador. See also B<laziness> and B<hubris>.
...
argument.) C<STDERR> is in the indirect object slot here:
    ...
    print STDERR "¡Despierta! ¡Despierta! Fear, Fire, Foes! ¡Despierta!\n";
...
Objeto de E/S interno. Can also mean B<indirect object>.
...
virtue of a programmer. Sin ella, no estaría leyendo esto. See also B<impatience> and
B<hubris>.
...
variable.  Cuando se hacen trampas con estas variables, suceden cosas mágicas.
...
I<man>(1) command. Una página de manual contiene una SINOPSIS, una DESCRIPCIÓN, una lista de ERRORES, etc., y generalmente no cabe en una sola página. There are manpages
...
big language, it turns out small. No hay quien lo entienda...
...
telephone’s telephone number. Generalmente una dirección IP. See also B<port>.
...
class it is. El programa puede pedir al objeto que haga cosas, pero es el objeto el que decide si las hace o no. Some objects are
more accommodating than others.

=item octal

A number X<octals>in base 8. Solo se admiten los dígitos 0 a 7. Las constantes octales en Perl empiezan por 0 (p. ej., 013). See also the C<oct> function.
...
Se suele usar también en un sentido más informal para indicar una cultura de programación específica. The loose sense can be used at varying levels of specificity.
...
about a file. En sistemas Unix puede consultar la página de manual I<ls>(1) para obtener más información.

=item Pern

What you get X<Pern (term)>when you do C<Perl++> twice. Doing it only once
will curl your hair. Debe incrementar ocho veces para lavar el pelo. Enjabonar, aclarar, repetir.
...
or at least priming it. Debe estar dispuesto a representar el papel de la Gran Calabaza cada cierto tiempo.
...
have a name. Algunos tipos comunes de referentes son los escalares, arrays, hashes o subrutinas.
...
ordering relationship is B<true> about a pair of B<operands>. Perl tiene operadores relaciones numéricos y de cadena. See B<collating sequence>.
...
database, or sent through a B<pipe>. En inglés también se llama I<marshalling>.
...
B<expression>. Actualmente se puede referir casi a cualquier cosa. For example,
...
Normalmente, esto solo influye en la eficiencia, no en la semántica. If your standard
...
Esto no existe. See B<lexical scoping>.
...
your system administrator. On Unix systems, the B<root> user. En sistemas Windows, generalmente el usuario Administrador.
...
shop in which to build special tools. Perl es una especie de taller de maquinaria.
...
Converting X<type casting>data from one type to another. C lo permite.
Perl does not need it. Ni lo desea.
...
El identificador de un usuario. X<UID (user ID)>X<user ID (UID)>Often used in the context of
