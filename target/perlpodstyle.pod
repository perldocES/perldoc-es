...
pages.  Por supuesto, todas estas directrices son opcionales, pero le ayudarán a crear una documentación coherente con otros documentos del sistema.
...
Arguments should be written in italics (IE<lt>E<gt>).  Los nombres de función se suelen escribir en cursiva; si escribe el nombre de una función como nombre(), Pod::Man se encargará de escribirlo en cursiva.  Literal code or commands should
...
A short usage summary for programs and functions.  Esta sección es obligatoria para páginas de sección 3.  For Perl module documentation, it's
...
a list using C<=over>, C<=item>, and C<=back>.  Por ejemplo:
