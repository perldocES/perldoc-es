...
¿Qué es una expresión regular?  A regular expression is simply a string
...
for use in regexp notation.  Los metacaracteres son
...
this, we would use the I<anchor> metacharacters C<^> and C<$>.  El ancla C<^> significa que coincide con el comienzo de la cadena y el ancla C<$> significa que coincide con el final de la cadena, o antes de un carácter de nueva línea al final de la cadena.  Here is how they are used:
...
to be possibly matched inside.  Aquí hay algunos ejemplos:
...
become the svelte C<[0-9]> and C<[a-z]>.  Algunos ejemplos son
...
those in the brackets.  Tanto C<[...]> como C<[^...]> deben coincidir con un carácter, o la coincidencia falla.  Entonces
...
of bracketed character classes.  Aquí hay algunos en uso:
...
match fails and Perl moves to the next position in the string.  Algunos ejemplos:
...
C<house> followed by either C<cat> or C<keeper>.  Algunos ejemplos más
    ...
    /house(cat(s|)|)/;  # matches either 'housecats' or 'housecat' or
                        # 'house'.  Note que los grupos pueden ser anidados.
...
Match 'a' followed by 'b'. Hasta aquí todo bien.
...
of the string after the match.  Un ejemplo:
...
the usual quantifiers with a C<?> appended to them.  Tienen los siguientes significados:
...
another solution. Tienen los siguientes significados:
...
the regexp in the I<last successful match> is used instead.  Por lo que tenemos
...
Una coincidencia fallida o cambiar la cadena objetivo reinicializa la posición.  If
...
Position 18 is good, but position 23 is bogus.  ¿Qué pasó?
...
que es la respuesta correcta.  This example illustrates that it is
...
string whatever is matched with the C<regexp>.  El operador C<=~> también se utiliza aquí para asociar una cadena con C<s///>.  Si está haciendo la búsqueda en C<$_>, el S<C<$_ =~>> puede ser omitido.  Si hay alguna coincidencia, C<s///> devuelve el número de sustituciones realizadas; de lo contrario devuelve falso.  Aquí hay algunos pocos ejemplos:
...
returned:
    ...
    print "$x $y\n"; # imprime "Me gustan los perros. I like dogs."
...
C<\Q>...C<\E> quotes, or protects most non-alphabetic characters.   Por ejemplo,
...
¡Menos mal! That is all the rest of the characters and character classes.
...
comment should not have any closing parentheses in the text.  Un ejemplo es
...
a regexp using C<(?i)>, C<(?m)>, C<(?s)>, and C<(?x)>.  Por ejemplo,
...
assertion is denoted by C<< (?<=fixed-regexp) >>.  Algunos ejemplos son
...
Hmm. ¿Qué pasó aquí? If you've been following along, you know that
...
effects of code expression execution are permanent.  Por lo tanto
...
completed capture group. Esto imprime

    It is a Fibonacci sequence
    Largest sequence matched was 5

¡Ja! Try that with your garden variety regexp package...
...
what the different parts of the debugging output tell you.  La primera parte
...
are I<experimental>.  Don't use them in production code.  Aún.
