...
Si está actualizando desde una versión anterior como la 5.10.0, primero lea L<perl5120delta>, que describe las diferencias entre 5.10.0 y la 5.12.0.
...
force scalar context on the argument.  Vea L<perlsub/Prototypes>.
...
El despacho de señales ha sido movido del bucle de ejecución a los operadores de control.
...
upgrade.  Vea los detalles a continuación.
...
core module dependency.  Esto ha sido corregido.
...
with analogous messages for getnet*() and getserv*().  Esto ha sido corregido.
...
Las subrutinas valor-izquierda pueden, nuevamente, devolver escalares por copia-en-escritura.  This
...
the stack, resulting in strange behaviour.  Por ejemplo,
...
C<List::Util::first> se porta mal en presencia de un C<$_> léxico (normalmente introducido por un C<my $_> o implícitamente por C<given>).  The variable
...
previously you could use them only on hashes.  Vea L<perlfunc> para más detalles.
...
was richer for his involvement.  Le echaremos de menos.
...
bug database at http://rt.perl.org/perlbug/ .  También puede haber información en http://www.perl.org/, la página oficial de Perl.
...
Si el fallo que está reportando tiene implicaciones de seguridad, que lo hace inadecuado para ser enviado a una lista de correo pública, por favor, envíelo a perl5-security-report@perl.org.  This points to a closed subscription
