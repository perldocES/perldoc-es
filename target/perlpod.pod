...
Es decir, con "=for" solo puede tener un párrafo de texto (p. ej., el texto en "=foo objetivo texto..."), pero con "=begin objetivo" ... "=end objetivo", puede tener cualquier cantidad de contenido entre los dos.  (Note that there still must be a blank line
...
until the end of its I<paragraph>, not its line.  Así, en los ejemplos anteriores, puede ver que cada comando necesita una línea en blanco después de él, para acabar su párrafo.  (And some older
...
Quiere decir que las palabras en I<texto> no deben quedar separadas en varias líneas.  Example: S<C<SE<lt>$x ? $y : $zE<gt>>>.
...
Esto su usa muy raramente.  Es una manera, algunas veces, de escapar código EE<lt>...E<gt>.  For example, instead of
...
Puede incrustar documentación Pod en sus módulos y guiones Perl.  Start
