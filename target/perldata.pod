...
El primer carácter del nombre nos indica el tipo de la estructura de datos.  El resto del nombre indica a qué valor concreto se accede.  Normalmente, este nombre es un I<identificador>, es decir, una palabra que comienza por una letra o un guión bajo, y que contiene letras, guiones bajos o dígitos.  En ciertos casos, puede ser una serie de identificadores separados por C<::> (o por el separador C<'>, algo anticuado), donde todos los identificadores menos el último se interpretan como nombres de paquetes que determinan el espacio de nombres en el que se buscará el identificador final (encontrará más detalles en la sección "Paquetes" de L<perlmod>).  Para una discusión más en profundidad sobre las expresiones regulares, vea L<Interpretación de identificadores>.  También es posible sustituir un identificador simple por una expresión que produzca una referencia al valor en tiempo de ejecución.   Esto se describe con mayor detalle más abajo y en L<perlref>.
...
=item Un sigilo, seguido únicamente por dígitos, coincidiendo con \p{POSIX_Digit}, como C<$0>, C<$1>, o C<$10000>.

=item Un sigilo, seguido tanto por un circunflejo y una única letra mayúscula POSIX, como C<$^V> o C<$^W>, o un sigilo seguido por un carácter de control literal coincidiendo con la propiedad C<\p{POSIX_Cntrl}>.
Debido a una rareza histórica, si no se está bajo C<use utf8>, los 128 controles extras en el rango C<[0x80-0xff]> también se pueden utilizar en variables de un solo carácter.  The use of a literal control
...
a string, and likewise within a hash subscript.  En ninguno de estos casos hay que usar comillas.  En un ejemplo anterior, C<$jornadas{'Feb'}> puede escribirse como C<$jornadas{Feb}> y las comillas se agregarán automáticamente.  Pero cualquier otra cosa más complicada en el subíndice se interpretará como una expresión.  Esto significa, por ejemplo, que C<$version{2.0}++> es equivalente a C<$version{2}++>, no a C<$version{'2.0'}++>.
