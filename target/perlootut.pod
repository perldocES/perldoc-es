...
La B<herencia> permite crear una versión especializada de una clase existente. Inheritance lets the new class reuse the methods and attributes
of another class.
...
Finally, we have L<Class::Tiny>. Este módulo se ajusta realmente a su nombre. It has an incredibly minimal API and absolutely no dependencies
on any recent Perl. Sin embargo, es mucho más sencillo usar este módulo que escribir código orientado a objetos desde cero.
...
With C<Class::Tiny>, all accessors are read-write. También genera automáticamente un constructor y todos los métodos de acceso que defina.
...
C<Moose> es la opción máxima (la más completa). Dispone de muchas características, un gran ecosistema y una importante base de usuarios. We also covered L<Moo> briefly.
...
C<Class::Tiny> is the absolute minimal option. No presenta dependencias y casi no hay sintaxis que aprender. Es una buena opción para entornos reducidos y para crear un programa rápidamente sin preocuparse de los detalles.

=item * L<Role::Tiny>

Use C<Role::Tiny> with C<Class::Accessor> or C<Class::Tiny> if you
find yourself considering multiple inheritance. Si se decanta por C<Moose>, podrá disponer de su implementación de roles.
